...
For the following Stata regression the coefficients of R translation and Stata don't match:
...
library(fixest)
rcmd = "feols"
cc_cols = c("group", "minority", "req_hs", "req_exp", "city", "august", "sec_man_con", "isicj", "sec_health_edu", "isicn", "isicm", "count_pagevisit")
cc_cols = intersect(cc_cols, colnames(dat))
dat = dat[complete.cases(dat[, cc_cols, drop=FALSE]), ]
formula = `count_pagevisit` ~ `group=2` + `group=3` + `req_hs` + `req_exp` + `city` + `august` + `sec_man_con` + `isicj` + `sec_health_edu` + `isicn` + `isicm`
ssc = fixest::ssc()
reg_vcov = "iid"
reg = feols(fml = formula,data = dat,vcov = reg_vcov,ssc = ssc)
```

Is perhaps the `i.minority*group_18m` term the culprit? Check with our pipeline and the Stata documentation.
