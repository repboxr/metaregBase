...
mrb_test_regcoeff_diff = function(
  ...
  compare_what_filter = match.arg(compare_what, c("all", "coef"))
  ...
  if (is.null(diff) || NROW(diff) == 0) return(NULL)

  diff = diff %>%
    filter(compare_what == compare_what_filter) %>%
    mutate(
  ...
  reg = parcels$reg
  regsource = parcels$regsource
  add_source_info = !is.null(regsource) && NROW(regsource) > 0
  ...
  txt = lapply(seq_len(NROW(diff)), function(i) {
    ...
    src_row = NULL
    if (add_source_info) {
      src_row = regsource[regsource$runid == runid, , drop = FALSE]
      if (NROW(src_row) == 0) src_row = NULL
    }
    ...
    diff_tab = NULL
    coef_stats = mrb_test_empty_coef_diff_stats()
    coef_text = "- Could not create coefficient comparison table."
    ...
    if (!is.null(coef_pair$co1) && !is.null(coef_pair$co2) &&
      ...
      coef_stats = mrb_test_coef_diff_stats(diff_tab)
      coef_text = mrb_test_regcoef_diff_text(
        diff_tab = diff_tab,
        variant1 = row$variant1,
        variant2 = row$variant2,
        compare_what = compare_what_filter,
        max_coef_cases = max_coef_cases
      )
    }
    ...
    summary_li = c(
      paste0(
        "- variants: ", row$variant1, " vs ", row$variant2,
        "; compare_what: ", row$compare_what
      ),
      paste0(
        "- summary: identical_share = ", mrb_test_fmt_num(row$identical_share),
        "; within_1pc_share = ", mrb_test_fmt_num(row$within_1pc_share),
        "; max_rel_diff = ", mrb_test_fmt_num(row$max_rel_diff),
        "; max_deviation = ", mrb_test_fmt_num(row$max_deviation)
      ),
      paste0(
        "- coef_only: identical_share = ", mrb_test_fmt_num(coef_stats$identical_share_coef),
        "; within_1pc_share = ", mrb_test_fmt_num(coef_stats$within_1pc_share_coef),
        "; max_rel_diff = ", mrb_test_fmt_num(coef_stats$max_rel_diff_coef),
        "; max_deviation = ", mrb_test_fmt_num(coef_stats$max_deviation_coef)
      )
    )

    if (isTRUE(row$ref_level_differ)) {
      summary_li = c(
        summary_li,
        "- note: reference levels differ, so part of the mismatch may come from different dummy dropping."
      )
    }
    if (!is.null(row$problem) && !is.na(row$problem) && nzchar(row$problem)) {
      summary_li = c(summary_li, paste0("- problem tag: ", row$problem))
    }

    if (!is.null(src_row) && NROW(src_row) > 0) {
      source_str = src_row$script_path[1]
      if ("line" %in% colnames(src_row) && !is.na(src_row$line[1])) {
        source_str = paste0(source_str, " (line ", src_row$line[1], ")")
      }
      summary_li = c(summary_li, paste0("- source: ", source_str))
    }
...
mrb_test_regcoef_diff_text = function(
  ...
  compare_what = match.arg(compare_what, c("all", "coef"))

  if (is.null(diff_tab) || NROW(diff_tab) == 0) {
    return("- No coefficient comparison rows available.")
  }

  tab = diff_tab %>%
    mutate(
      coef_missing = xor(is.na(coef_1), is.na(coef_2)),
      se_missing = xor(is.na(se_1), is.na(se_2)),
      has_coef_diff = coef_missing | (!is.na(abs_err_coef) & abs_err_coef > 0),
      has_se_diff = se_missing | (!is.na(abs_err_se) & abs_err_se > 0),
      has_any_diff = has_coef_diff | has_se_diff,
      rel_rank_coef = ifelse(coef_missing, Inf, ifelse(is.na(rel_err_coef), -Inf, rel_err_coef)),
      abs_rank_coef = ifelse(coef_missing, Inf, ifelse(is.na(abs_err_coef), -Inf, abs_err_coef)),
      rel_rank_all = ifelse(
        coef_missing | se_missing,
        Inf,
        pmax(
          ifelse(is.na(rel_err_coef), -Inf, rel_err_coef),
          ifelse(is.na(rel_err_se), -Inf, rel_err_se)
        )
      ),
      abs_rank_all = ifelse(
        coef_missing | se_missing,
        Inf,
        pmax(
          ifelse(is.na(abs_err_coef), -Inf, abs_err_coef),
          ifelse(is.na(abs_err_se), -Inf, abs_err_se)
        )
      )
    )

  if (compare_what == "coef") {
    tab = tab %>%
      filter(has_coef_diff) %>%
      arrange(desc(coef_missing), desc(rel_rank_coef), desc(abs_rank_coef), cterm)
  } else {
    tab = tab %>%
      filter(has_any_diff) %>%
      arrange(desc(coef_missing | se_missing), desc(rel_rank_all), desc(abs_rank_all), cterm)
  }

  if (NROW(tab) == 0) {
    return("- No differing coefficients to show.")
  }

  if (is.finite(max_coef_cases) && NROW(tab) > max_coef_cases) {
    tab = tab[seq_len(max_coef_cases), , drop = FALSE]
  }
  ...
  paste0(
    "- Top ", NROW(show), " mismatches shown",
    if (compare_what == "coef") " (ordered by coefficient differences)." else " (ordered by coefficient and standard error differences).",
    "\n```text\n",
    out,
    "\n```"
  )
...
mrb_run_as_test = function(project_dir, run_script, check_reg=TRUE, navigate=TRUE) {
  ...
  add("Report of test run for ", project_dir)
...
These packages are part of a workflow to allow robustness replications of regressions originally run in Stata also in R. Goal is to build a systemat regression database.

Currently, repboxDRF allows to generate R code that modifies the data set in R to have the state as before the regression command.

I need an additional feature: 

Also get a filtered version of the data set, that applies the "if" and "in" filters used in the Stata regression.

I don't want to reinvent the wheel.

What is the best way to achieve it without much extra code.

E.g. in the translation pipeline we could add a translate of a custom generated Stata "keep" command that select the rows based on filters from an if and in condition. 

I don't know whether stata2r already systematically extracts the in and if conditions or has implemented something.

cmdpart has a nicer parser from which we could extract the "if" or "in" conditions otherwise.

Please suggest and discuss different approaches. Do not code yet.
