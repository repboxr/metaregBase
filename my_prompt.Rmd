...
### source_with_log start: 2026-03-23 08:16:41 
...
mrb_r_base processing runids: 3 5 Error in `dplyr::mutate()`:
ℹ In argument: `u_sample = as.integer(e(sample))`.
Caused by error in `e()`:
...
25: .handleSimpleError(function (cnd) 
    ...
    }, "could not find function \"e\"", base::quote(e(sample)))
...
18: mutate.data.frame(., u_sample = as.integer(e(sample)))
17: dplyr::mutate(., u_sample = as.integer(e(sample)))
16: data %>% dplyr::mutate(u_sample = as.integer(e(sample))) at <text>#1
...
13: scmd_generate(data = data, new_var = "u_sample", r_expr_str = "e(`sample`)", 
        is_string = FALSE, force_integer = TRUE) at <text>#3
12: eval(rcode_call, envir = exec_env)
11: eval(rcode_call, envir = exec_env) at drf_run_r.R#86
...
I just recently updated stata2r. E.g. it has now a function that marks regression commands that perform data manipulation or generate e() variables that are later required, it is e.g. called in main.R
...
but I think there is also an external function that can be directly called.

Propose how to update repboxDRF and metaregBase to account for it, e.g.
when deciding which regression commands need to be added when Stata code that prepares the data is generated.

I also thought stata2r performs the translation of e() functions, I am not completely sure why the error in the test report still arises. Propose a solution.
