...
mrb_test_regcoef_diff_text = function(
  ...
  # Safety filter: remove expected missing intercepts before compiling the text report
  cmd_col = if ("cmd_1" %in% names(diff_tab)) "cmd_1" else if ("cmd" %in% names(diff_tab)) "cmd" else NULL
  if (!is.null(cmd_col) && variant2 == "rb") {
    diff_tab = diff_tab %>%
      filter(!(cterm == "(Intercept)" & !is.na(.data[[cmd_col]]) & .data[[cmd_col]] %in% mrb_cmds_ignore_intercept_in_r()))
  }

  if (NROW(diff_tab) == 0) {
    return(list(text = "- No differing coefficients to show.", note = ""))
  }
...
Can you update the metareg base test report created in

mrb_test_report = function(project_dir, parcels, drf, opts=mrb_test_opts())?

1. The current overview of problems is too brief. Example: 

```
In 6 of 20 regressions, problems were detected. 1 cases are shown below.

## runid 130

**Issues detected:** Metareg Stata (sb) and R (rb) differ
```
  Please make a list by problem type combination and note the runid that have this problem type combination. E.g.
  
```
Problem types and runids:

  - Coef differ & SE differ: 103, 206
  - SE differ: 25, 56, 37 
```

2. Where is the parameter that define how many problem examples are shown? Typically pick an example that has the most severe problem type (wrong coefficients worse than wrong SE)

3. The diff_tab argument passed to mrb_test_regcoef_diff_text seems typically to have no cmd1 or cmd2 columns. Here are the colnames from the last restore point from our example. 

> names(diff_tab)
 [1] "runid"               "eq"                  "cterm"               "identical"          
 [5] "identical_coef"      "variant_1"           "shown_term_1"        "label_1"            
 [9] "coef_1"              "se_1"                "t_1"                 "p_1"                
[13] "ci_low_1"            "ci_up_1"             "variant_2"           "shown_term_2"       
[17] "label_2"             "coef_2"              "se_2"                "t_2"                
[21] "p_2"                 "ci_low_2"            "ci_up_2"             "is_ia"              
[25] "is_factor"           "factor_group"        "ref_level_differs"   "offset.2"           
[29] "num_diff_ref_coef_2" "offset.2.intercept"  "abs_err_coef"        "abs_err_se"         
[33] "rel_err_coef"        "rel_err_se"          "rel_within_1pc_coef" "rel_within_1pc"     
[37] "step_refs_differ"   

Therefore the intercept removal from areg commands etc does not work. Generally, when Stata commands like areg are translated to R (using fixest), we will not expect that the intercept matches, or more precisely fixest will not even have an intercept. This should not be marked as a problem. But currently the line

coef_missing_one = xor(is.na(coef_1), is.na(coef_2)),

yields a TRUE for the intercept even with areg commands as they are not removed.
