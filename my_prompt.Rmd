...
### source_with_log start: 2026-03-24 12:31:46 
...
mrb_r_base processing runids: 3 5 12 18 19 27 28 31 40 Error in `dplyr::mutate()`:
ℹ In argument: `x1_capped = as.numeric(sfun_stata_cond(x1 > cutoff, cutoff,
  x1))`.
...
33: abort(message, class = error_class, parent = parent, call = error_call)
...
31: .handleSimpleError(function (cnd) 
    ...
    }, "object 'cutoff' not found", base::quote(as.list.environment(envir, 
        all.names = TRUE)))
30: as.list.environment(envir, all.names = TRUE)
29: as.list(envir, all.names = TRUE)
28: copy.fun(envir)
27: store.objects(name = name, parent.num = -2, deep.copy = deep.copy, 
        force = force, dots = dots)
26: restore.point("sfun_stata_cond") at sfun_stata_cond.R#5
25: sfun_stata_cond(x1 > cutoff, cutoff, x1)
...
18: mutate.data.frame(., x1_capped = as.numeric(sfun_stata_cond(x1 > 
        cutoff, cutoff, x1)))
17: dplyr::mutate(., x1_capped = as.numeric(sfun_stata_cond(x1 > 
        cutoff, cutoff, x1)))
16: data %>% dplyr::mutate(x1_capped = as.numeric(sfun_stata_cond(x1 > 
        cutoff, cutoff, x1))) at <text>#1
...
14: eval(parse(text = paste(pipe_el, collapse = " %>% ")), envir = list(data = data), 
        enclos = parent.frame()) at t_generate.R#79
13: scmd_generate(data = data, new_var = "x1_capped", r_expr_str = "sfun_stata_cond(`x1` > `cutoff`,  `cutoff`,  `x1`)", 
        is_string = FALSE, force_integer = FALSE) at <text>#11
12: eval(rcode_call, envir = exec_env)
11: eval(rcode_call, envir = exec_env) at drf_run_r.R#85
10: repboxDRF::drf_get_data(pid, drf = mrb$drf) at mrb_r_base.R#96
9: mrb_run_r_base_step(mrb, pid) at mrb_r_base.R#50
8: mrb_run_r_base(mrb) at run_test.R#33
7: eval(ei, envir)
6: eval(ei, envir)
...
1: metaregBase::mrb_run_as_test(project_dir = "~/repbox/projects/test", 
       "~/repbox/projects/test/run/run_test.R", opts = opts)
```
...
Would additional information be helpful to pinpoint the bug? Yes, what?

How would you improve the code that generates the test report?
