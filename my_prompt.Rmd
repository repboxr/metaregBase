...
Currently, I have regression commands like reg3 that I classify as quasi_reg because either cmdpart parsing does not work or I just don't want to apply the metaregbase pipeline on them (e.g. because too time consuming). But for quasi_reg also no original reproduction outcome in form of parmest coefficients is stored and for some of those "quasi_reg" regressions such storage would work perfectly well. 

I think of introducing a new category "no_mrb_reg" for regressions where we can store systematic info from the first reproduction run but not perform any further mrb analysis and neither build drf paths. In repboxDRF I would simply like to add a function no_mrb_reg_cmds.

cmds noted there shall have the cmd_type to no_mrb_reg

Can you check whether that could be well implemented? Describe first your plan of changes and then propose the changes. Check that no subsequent bugs will be introduced e.g. when creating the regcheck parcel. 
