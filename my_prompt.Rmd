...
Currently, the representation and translation of weighted regressions is based on the assumption that in Stata one can only specify the weight variable. Now I found out that weights can also be expressions. E.g. all the commands below specify weighted regressions:
...
* But we can also use formulas as weights
reg y x1 [aw = 1/w]
reg y x1 [weight=w*w2]
reg y x1 [weight=exp(w*w2)]
```

Incorporating weight expressions in our pipeline seems a bigger structural change. Currently weights are in regvar etc but assume only a single variable. 

Basic idea: distinguish formula and variable weights. For formula weights generally try to generate a specific weights variable data$.weight in the regression translation code. In regvar store all variables that influence the weights...

First make one or multiple detailed proposals how we could rework our pipeline using weights. Discuss pros and cons. No code yet, just conceptual discussion.
