...
I have the following example in an do file:

quietly reg ln_pd1500 pc_lnar_lnas ln_abslat distcr1000 land100cr africa europe asia americas if cleanpd1500==1, robust ;


It is stored in stata_run_cmd with cmd "quietly" instead of "reg" and not marked as a regression. Actually repboxStata should already account for prefixes quietly, capture, noisily and just mark them as special prefix. But did not work here. Maybe a bug was introduced recently that changed this behavior. Can you check, explain and propose a fix (also explain your fix)? Try to change only what is necessary.
