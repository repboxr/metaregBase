...
I have a bug reproduction package. For some reason some regression commands seem to be missing from the original Stata reproduction.

These are regression from Table8.do. Here the start
...
use $root/data/EU_data.dta, clear

su hc if growth!=. & gdpcap!=. 
replace hc =hc-r(mean)

su qog if growth!=. & gdpcap!=. 
replace qog =qog-r(mean)

****************************************
***********  First Stage    ************
****************************************

global vlist_poly5  "gdpcap_5 gdpcap_4 gdpcap_3 gdpcap_2 gdpcap"
global vlist_i_poly5  "gdpcap_i_5 gdpcap_i_4 gdpcap_i_3 gdpcap_i_2 gdpcap_i"

global instruments1 "literacy1870 protestant capital_labor_85"
global instruments2 "literacy1870 protestant"
global instruments3 "literacy1870 capital_labor_85"
global instruments4 "literacy1870 protestant"
global instruments5 "protestant"

reg hc ${instruments1} ${vlist_poly5} ${vlist_i_poly5} 
reg hc ${instruments2} ${vlist_poly5} ${vlist_i_poly5} 
reg hc ${instruments3} ${vlist_poly5} ${vlist_i_poly5} 

reg qog ${instruments4} ${vlist_poly5} ${vlist_i_poly5} 
reg qog ${instruments5} ${vlist_poly5} ${vlist_i_poly5} 

save $root\data\temp.dta, replace
****************************************
***********  Second Stage   ************
****************************************
**Columns 1-3: Human Capital**
**Point Estimate**
foreach num in 1 2 3	{
```

All the listed regressions here:
...
Are missing already from stata_run_cmd. Later regressions and commands exist again. Below is an except from the log file.

Table8.do is called from master.do:
...
do "$root\stata\Table1.do" 
do "$root\stata\Table2.do"
do "$root\stata\Table3.do"
do "$root\stata\Table4.do"
do "$root\stata\Table8.do"    
```


The screenshot also illustrates the game in stata_run_cmd at those regressions. Can you find out what is the problem, explain and provide a fix? Likely something in the repboxStata or repboxStataReg package?
...
-------------------------------------------------------------------------------
      name:  repbox_log_1
       log:  /__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2/repbox/stata/logs/include
> _1.log
  log type:  text
 opened on:  30 Jun 2026, 11:14:26
. }
...
. local repbox_cmdline = `"version 9"'

. 
. file open repbox_cmdlines using "/__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2//re
> pbox/stata/cmd/precmd_1.csv", write append
(file /__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2//repbox/stata/cmd/precmd_1.csv
    not found)
...
#~# INJECT RUNCMD 1 1 979

. capture:  noisily: version 9

. 
. display "#~# END INJECT RUNCMD 1 1 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 1 979
...
#~# INJECT RUNCMD 1 2 980

. capture:  noisily: clear

. 
. display "#~# END INJECT RUNCMD 1 2 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 2 980

. 
. * CLEAR INJECTION START
. 
. 
. qui {
...
. * INJECTION END
. 
. *capture:  noisily: capture log close
. 
...
. local repbox_cmdline = `"capture set virtual off"'
...
#~# INJECT RUNCMD 1 4 981

. capture:  noisily: capture set virtual off

. 
. display "#~# END INJECT RUNCMD 1 4 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 4 981
...
. local repbox_cmdline = `"capture set memory 400m"'
...
#~# INJECT RUNCMD 1 5 982

. capture:  noisily: capture set memory 400m

. 
. display "#~# END INJECT RUNCMD 1 5 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 5 982
...
. local repbox_cmdline = `"capture set matsize 800"'
...
#~# INJECT RUNCMD 1 6 983

. capture:  noisily: capture set matsize 800

. 
. display "#~# END INJECT RUNCMD 1 6 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 6 983
...
. local repbox_cmdline = `"capture set more off"'
...
#~# INJECT RUNCMD 1 7 984

. capture:  noisily: capture set more off

. 
. display "#~# END INJECT RUNCMD 1 7 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 7 984
...
#~# INJECT RUNCMD 1 8 985

. capture:  noisily: memory
...
. local repbox_cmdline = `"set seed 12345"'
...
#~# INJECT RUNCMD 1 9 986

. capture:  noisily: set seed 12345

. 
. display "#~# END INJECT RUNCMD 1 9 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 9 986
...
. local repbox_cmdline = `"use $root/data/EU_data.dta, clear"'

. 
. 
. local repbox_source_path = subinstr("$root/data/EU_data.dta","\","/",.)

. 
. repbox_correct_path "file_exists" "`repbox_source_path'" "dta" "/__w/gha_rp/g
> ha_rp/gha_work/aejpol_5_4_2/mod" "/__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2/mo
> d/data/stata"
  /__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2/mod/data/data/EU_data.dta
...
. local repbox_cmdline = `"su hc if growth!=. & gdpcap!=."'
...
#~# INJECT RUNCMD 1 11 988

. capture:  noisily: su hc if growth!=. & gdpcap!=.

    Variable |        Obs        Mean    Std. dev.       Min        Max
-------------+---------------------------------------------------------
          hc |        646    .5458498    .1438512   .1703423   .8635327

. 
. display "#~# END INJECT RUNCMD 1 11 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 11 988
...
. local repbox_cmdline = `"replace hc =hc-r(mean)"'
...
#~# INJECT RUNCMD 1 12 989

. capture:  noisily: replace hc =hc-r(mean)
(653 real changes made)

. 
. display "#~# END INJECT RUNCMD 1 12 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 12 989
...
. local repbox_cmdline = `"su qog if growth!=. & gdpcap!=."'
...
#~# INJECT RUNCMD 1 13 990

. capture:  noisily: su qog if growth!=. & gdpcap!=.

    Variable |        Obs        Mean    Std. dev.       Min        Max
-------------+---------------------------------------------------------
         qog |        640    .4383006     .815335   -2.31818    1.62489

. 
. display "#~# END INJECT RUNCMD 1 13 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 13 990
...
. local repbox_cmdline = `"replace qog =qog-r(mean)"'
...
#~# INJECT RUNCMD 1 14 991

. capture:  noisily: replace qog =qog-r(mean)
(659 real changes made)

. 
. display "#~# END INJECT RUNCMD 1 14 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 14 991
...
. local repbox_cmdline = `"global vlist_poly5  "gdpcap_5 gdpcap_4 gdpcap_3 gdpc
> ap_2 gdpcap""'
...
#~# INJECT RUNCMD 1 15 992

. capture:  noisily: global vlist_poly5  "gdpcap_5 gdpcap_4 gdpcap_3 gdpcap_2 g
> dpcap"

. 
. display "#~# END INJECT RUNCMD 1 15 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 15 992
...
. local repbox_cmdline = `"global vlist_i_poly5  "gdpcap_i_5 gdpcap_i_4 gdpcap_
> i_3 gdpcap_i_2 gdpcap_i""'
...
#~# INJECT RUNCMD 1 16 993

. capture:  noisily: global vlist_i_poly5  "gdpcap_i_5 gdpcap_i_4 gdpcap_i_3 gd
> pcap_i_2 gdpcap_i"

. 
. display "#~# END INJECT RUNCMD 1 16 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 16 993
...
. local repbox_cmdline = `"global instruments1 "literacy1870 protestant capital
> _labor_85""'
...
#~# INJECT RUNCMD 1 17 994

. capture:  noisily: global instruments1 "literacy1870 protestant capital_labor
> _85"

. 
. display "#~# END INJECT RUNCMD 1 17 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 17 994
...
. local repbox_cmdline = `"global instruments2 "literacy1870 protestant""'
...
#~# INJECT RUNCMD 1 18 995

. capture:  noisily: global instruments2 "literacy1870 protestant"

. 
. display "#~# END INJECT RUNCMD 1 18 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 18 995
...
. local repbox_cmdline = `"global instruments3 "literacy1870 capital_labor_85""
> '
...
#~# INJECT RUNCMD 1 19 996

. capture:  noisily: global instruments3 "literacy1870 capital_labor_85"

. 
. display "#~# END INJECT RUNCMD 1 19 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 19 996
...
. local repbox_cmdline = `"global instruments4 "literacy1870 protestant""'
...
#~# INJECT RUNCMD 1 20 997

. capture:  noisily: global instruments4 "literacy1870 protestant"

. 
. display "#~# END INJECT RUNCMD 1 20 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 20 997
...
. local repbox_cmdline = `"global instruments5 "protestant""'
...
#~# INJECT RUNCMD 1 21 998

. capture:  noisily: global instruments5 "protestant"

. 
. display "#~# END INJECT RUNCMD 1 21 `repbox_local_cmd_count'
#~# END INJECT RUNCMD 1 21 998
...
. 
. capture:  noisily: reg hc ${instruments1} ${vlist_poly5} ${vlist_i_poly5}

      Source |       SS           df       MS      Number of obs   =       646
-------------+----------------------------------   F(13, 632)      =    175.15
       Model |  10.4472357        13  .803633512   Prob > F        =    0.0000
    Residual |  2.89985758       632  .004588382   R-squared       =    0.7827
-------------+----------------------------------   Adj R-squared   =    0.7783
       Total |  13.3470932       645  .020693168   Root MSE        =    .06774

------------------------------------------------------------------------------
          hc | Coefficient  Std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
literacy1870 |   .0027148   .0001283    21.15   0.000     .0024628    .0029669
  protestant |   .1389308   .0124223    11.18   0.000     .1145368    .1633248
capital_l~85 |   .0190839   .0012517    15.25   0.000      .016626    .0215419
    gdpcap_5 |   8.94e-23   4.43e-22     0.20   0.840    -7.80e-22    9.59e-22
    gdpcap_4 |  -7.08e-18   2.48e-17    -0.28   0.776    -5.59e-17    4.17e-17
    gdpcap_3 |   2.02e-13   4.79e-13     0.42   0.673    -7.39e-13    1.14e-12
    gdpcap_2 |  -2.48e-09   3.78e-09    -0.66   0.511    -9.91e-09    4.94e-09
      gdpcap |   .0000127   .0000115     1.10   0.272    -9.97e-06    .0000354
  gdpcap_i_5 |   9.46e-21   8.25e-20     0.11   0.909    -1.52e-19    1.71e-19
  gdpcap_i_4 |   2.98e-16   1.71e-15     0.17   0.862    -3.07e-15    3.66e-15
  gdpcap_i_3 |   3.72e-12   1.25e-11     0.30   0.766    -2.09e-11    2.83e-11
  gdpcap_i_2 |   2.81e-08   3.52e-08     0.80   0.424    -4.09e-08    9.72e-08
    gdpcap_i |   .0000512   .0000452     1.13   0.258    -.0000376    .0001399
       _cons |  -.2196134   .0114622   -19.16   0.000    -.2421221   -.1971047
------------------------------------------------------------------------------

. capture:  noisily: reg hc ${instruments2} ${vlist_poly5} ${vlist_i_poly5}

      Source |       SS           df       MS      Number of obs   =       646
-------------+----------------------------------   F(12, 633)      =    124.75
       Model |  9.38060955        12  .781717462   Prob > F        =    0.0000
    Residual |  3.96648369       633  .006266167   R-squared       =    0.7028
-------------+----------------------------------   Adj R-squared   =    0.6972
       Total |  13.3470932       645  .020693168   Root MSE        =    .07916

------------------------------------------------------------------------------
          hc | Coefficient  Std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
literacy1870 |   .0035913   .0001341    26.78   0.000      .003328    .0038546
  protestant |   .1379522   .0145167     9.50   0.000     .1094455     .166459
    gdpcap_5 |  -2.12e-22   5.17e-22    -0.41   0.682    -1.23e-21    8.03e-22
    gdpcap_4 |   1.25e-17   2.90e-17     0.43   0.667    -4.45e-17    6.94e-17
    gdpcap_3 |  -2.48e-13   5.59e-13    -0.44   0.658    -1.35e-12    8.50e-13
    gdpcap_2 |   1.96e-09   4.40e-09     0.44   0.657    -6.69e-09    1.06e-08
      gdpcap |  -6.22e-06   .0000134    -0.46   0.643    -.0000326    .0000201
  gdpcap_i_5 |  -2.74e-19   9.39e-20    -2.92   0.004    -4.58e-19   -8.95e-20
  gdpcap_i_4 |  -5.78e-15   1.95e-15    -2.97   0.003    -9.61e-15   -1.96e-15
  gdpcap_i_3 |  -4.01e-11   1.42e-11    -2.82   0.005    -6.80e-11   -1.22e-11
  gdpcap_i_2 |  -1.03e-07   3.98e-08    -2.57   0.010    -1.81e-07   -2.43e-08
    gdpcap_i |  -.0000647   .0000521    -1.24   0.215    -.0001669    .0000376
       _cons |  -.2488939   .0132056   -18.85   0.000    -.2748259   -.2229618
------------------------------------------------------------------------------

. capture:  noisily: reg hc ${instruments3} ${vlist_poly5} ${vlist_i_poly5}

      Source |       SS           df       MS      Number of obs   =       646
-------------+----------------------------------   F(12, 633)      =    149.93
       Model |   9.8733165        12  .822776375   Prob > F        =    0.0000
    Residual |  3.47377674       633  .005487799   R-squared       =    0.7397
-------------+----------------------------------   Adj R-squared   =    0.7348
       Total |  13.3470932       645  .020693168   Root MSE        =    .07408

------------------------------------------------------------------------------
          hc | Coefficient  Std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
literacy1870 |   .0031123   .0001349    23.08   0.000     .0028475    .0033772
capital_l~85 |   .0190116   .0013688    13.89   0.000     .0163236    .0216996
    gdpcap_5 |   2.19e-22   4.84e-22     0.45   0.651    -7.32e-22    1.17e-21
    gdpcap_4 |  -1.57e-17   2.72e-17    -0.58   0.563    -6.90e-17    3.76e-17
    gdpcap_3 |   4.12e-13   5.24e-13     0.79   0.432    -6.16e-13    1.44e-12
    gdpcap_2 |  -4.74e-09   4.13e-09    -1.15   0.251    -1.29e-08    3.36e-09
      gdpcap |   .0000226   .0000126     1.79   0.074    -2.16e-06    .0000473
  gdpcap_i_5 |   1.63e-20   9.02e-20     0.18   0.857    -1.61e-19    1.93e-19
  gdpcap_i_4 |   5.07e-16   1.87e-15     0.27   0.787    -3.17e-15    4.19e-15
  gdpcap_i_3 |   5.22e-12   1.37e-11     0.38   0.703    -2.17e-11    3.21e-11
  gdpcap_i_2 |   3.56e-08   3.84e-08     0.93   0.355    -3.99e-08    1.11e-07
    gdpcap_i |   .0000489   .0000494     0.99   0.322    -.0000481     .000146
       _cons |  -.2193802   .0125354   -17.50   0.000    -.2439962   -.1947642
------------------------------------------------------------------------------

. capture:  noisily: reg qog ${instruments4} ${vlist_poly5} ${vlist_i_poly5}

      Source |       SS           df       MS      Number of obs   =       640
-------------+----------------------------------   F(12, 627)      =     76.35
       Model |  252.193668        12   21.016139   Prob > F        =    0.0000
    Residual |  172.595081       627  .275271262   R-squared       =    0.5937
-------------+----------------------------------   Adj R-squared   =    0.5859
       Total |  424.788749       639  .664771125   Root MSE        =    .52466

------------------------------------------------------------------------------
         qog | Coefficient  Std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
literacy1870 |   .0114545    .000889    12.88   0.000     .0097087    .0132003
  protestant |   1.298313   .0962291    13.49   0.000     1.109343    1.487284
    gdpcap_5 |   8.09e-21   3.43e-21     2.36   0.019     1.36e-21    1.48e-20
    gdpcap_4 |  -4.71e-16   1.92e-16    -2.45   0.015    -8.48e-16   -9.35e-17
    gdpcap_3 |   9.23e-12   3.70e-12     2.49   0.013     1.96e-12    1.65e-11
    gdpcap_2 |  -7.03e-08   2.92e-08    -2.41   0.016    -1.28e-07   -1.29e-08
      gdpcap |   .0001624    .000089     1.83   0.068    -.0000122    .0003371
  gdpcap_i_5 |   1.96e-18   6.35e-19     3.09   0.002     7.13e-19    3.21e-18
  gdpcap_i_4 |   4.54e-14   1.32e-14     3.45   0.001     1.95e-14    7.13e-14
  gdpcap_i_3 |   3.43e-10   9.61e-11     3.57   0.000     1.54e-10    5.32e-10
  gdpcap_i_2 |   1.15e-06   2.68e-07     4.27   0.000     6.18e-07    1.67e-06
    gdpcap_i |   .0009743   .0003465     2.81   0.005     .0002939    .0016548
       _cons |   -.879377   .0875482   -10.04   0.000      -1.0513   -.7074537
------------------------------------------------------------------------------

. capture:  noisily: reg qog ${instruments5} ${vlist_poly5} ${vlist_i_poly5}

      Source |       SS           df       MS      Number of obs   =       640
-------------+----------------------------------   F(11, 628)      =     54.01
       Model |  206.495622        11  18.7723293   Prob > F        =    0.0000
    Residual |  218.293127       628   .34760052   R-squared       =    0.4861
-------------+----------------------------------   Adj R-squared   =    0.4771
       Total |  424.788749       639  .664771125   Root MSE        =    .58958

------------------------------------------------------------------------------
         qog | Coefficient  Std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
  protestant |    1.67855   .1029245    16.31   0.000     1.476432    1.880668
    gdpcap_5 |   9.59e-21   3.85e-21     2.49   0.013     2.03e-21    1.71e-20
    gdpcap_4 |  -5.72e-16   2.16e-16    -2.65   0.008    -9.96e-16   -1.49e-16
    gdpcap_3 |   1.17e-11   4.16e-12     2.82   0.005     3.56e-12    1.99e-11
    gdpcap_2 |  -9.80e-08   3.27e-08    -2.99   0.003    -1.62e-07   -3.37e-08
      gdpcap |   .0003111   .0000991     3.14   0.002     .0001165    .0005058
  gdpcap_i_5 |   9.13e-19   7.07e-19     1.29   0.197    -4.76e-19    2.30e-18
  gdpcap_i_4 |   2.30e-14   1.47e-14     1.57   0.117    -5.77e-15    5.18e-14
  gdpcap_i_3 |   1.80e-10   1.07e-10     1.68   0.094    -3.06e-11    3.90e-10
  gdpcap_i_2 |   7.48e-07   3.00e-07     2.50   0.013     1.60e-07    1.34e-06
    gdpcap_i |   .0004701   .0003869     1.22   0.225    -.0002896    .0012298
       _cons |  -.4787012   .0919646    -5.21   0.000    -.6592966   -.2981058
------------------------------------------------------------------------------

. 
. * PRECOMAND INJECTION START LINE 27
. 
. global repbox_cmd_count = $repbox_cmd_count +1

. local repbox_local_cmd_count = $repbox_cmd_count

. local repbox_cmdline = `"save $root\data\temp.dta, replace"'

. 
. 
. local repbox_source_path = subinstr("$root\data\temp.dta","\","/",.)

. 
. repbox_correct_path "file_create" "`repbox_source_path'" "dta" "/__w/gha_rp/g
> ha_rp/gha_work/aejpol_5_4_2/mod" "/__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2/mo
> d/data/stata"
  /__w/gha_rp/gha_rp/gha_work/aejpol_5_4_2/mod/data/temp.dta
...
. file write repbox_cmdlines `"1;27;`repbox_local_cmd_count';$repbox_root_donum
> ;$S_TIME;`c(pwd)';`r(repbox_corrected_path)';`repbox_cmdline'"'
...
. 
. * PRECOMMAND INJECTION END
. display "#~# INJECT RUNCMD 1 27 `repbox_local_cmd_count'
#~# INJECT RUNCMD 1 27 999
