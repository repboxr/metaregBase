...
Currently, my regression translation does not account for a noconst option in a Stata regression. Consider the following Stata code:

reg y x1, noconst

It run the regression without constant. Currently it is translated to an R regression with constant. Also, while the cmdpart parcel contains the options and the regcoef parcel misses the constant the noconst flag is not very transparent in our data base.

Please check and discuss the following update proposal:

- I added a new field `flags`  to the reg parcel:

```
  flags:
    descr: |
      Additional flags we may store for the regression
      as a comma separated string. Currently:

      noconst: regression was specified without a constant
```

- By default reg$flag = "" (must be added in code that generates parcels), but metaregBase shall ensure that the `noconst` is added to flags for regressions with noconst argument.

- We should pass that information to regtranslate in a suitable fashion and it should adapt the R code for regressions without constants.
