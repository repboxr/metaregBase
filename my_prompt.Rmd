...
Does repboxRun have a report function that shows the state of the current reproduction as an markdown file? If not can you create such a function? If sensible you can add a helper function in metaregBase that shows infos about the metaregBase reproduction or you can put everything into repboxRun. The function should be able to run for every level of reproduction, show what has been done, note issues / problems and provide overall statistics. 

Output format should be markdown. If no report infrastructure exists, put it into a new R file in repboxRun.
