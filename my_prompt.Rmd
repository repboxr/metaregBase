...
. do "repbox_probit_logit.do", nostop 
...
> g
  log type:  text
 opened on:  28 Apr 2026, 10:20:15
.   adopath + "~/ado/plus"
...
. local repbox_cmdline = `"probit d1 x1 c.x1#i.i1, vce(robust)"'
...
. capture:  noisily: probit d1 x1 c.x1#i.i1, vce(robust)

Iteration 0:   log pseudolikelihood = -69.294717  
Iteration 1:   log pseudolikelihood = -65.414813  
Iteration 2:   log pseudolikelihood = -65.400418  
Iteration 3:   log pseudolikelihood = -65.400415  

Probit regression                                       Number of obs =    100
                                                        Wald chi2(4)  =   9.38
                                                        Prob > chi2   = 0.0523
Log pseudolikelihood = -65.400415                       Pseudo R2     = 0.0562

------------------------------------------------------------------------------
             |               Robust
          d1 | Coefficient  std. err.      z    P>|z|     [95% conf. interval]
-------------+----------------------------------------------------------------
          x1 |  -.3904321   .2967356    -1.32   0.188    -.9720232     .191159
             |
     i1#c.x1 |
          2  |    .973565   .3681635     2.64   0.008     .2519777    1.695152
          3  |   .2736179   .4464989     0.61   0.540    -.6015039     1.14874
          4  |   .5426026   .3961216     1.37   0.171    -.2337815    1.318987
             |
       _cons |  -.0631297   .1305146    -0.48   0.629    -.3189336    .1926742
------------------------------------------------------------------------------
...
. * REGRESSION INJECTION START
. if (`repbox_reg_rc' == 0) {
.   parmest, label saving("~/repbox/projects_test/test/repbox/stata/tsv/1_2_`re
> pbox_local_cmd_count'.dta", replace)
...
scalars:
          ...
          e(converged) =  1
                 e(rc) =  0
                 e(ll) =  -65.40041531870908
         e(k_eq_model) =  1
               e(ll_0) =  -69.29471672244782
               e(df_m) =  4
               e(chi2) =  9.37896626212077
                  e(p) =  .0522945183444159
              e(N_cdf) =  0
              e(N_cds) =  0
               e(r2_p) =  .0561991099456676

macros:
            e(cmdline) : "probit d1 x1 c.x1#i.i1, vce(robust)"
                e(cmd) : "probit"
...
. else {
.   display "#~# REGRESSION COMMAND FAILED 1 2 `repbox_local_cmd_count' rc=`rep
> box_reg_rc'"
. }
...
. local repbox_cmdline = `"gen byte probit_sample = e(sample)"'
...
. capture:  noisily: gen byte probit_sample = e(sample)
...
. local repbox_cmdline = `"logit d2 x1 if probit_sample == 1, vce(cluster i)"'
...
. capture:  noisily: logit d2 x1 if probit_sample == 1, vce(cluster i)

Iteration 0:   log pseudolikelihood = -69.294717  
Iteration 1:   log pseudolikelihood = -68.790612  
Iteration 2:   log pseudolikelihood = -68.790565  
Iteration 3:   log pseudolikelihood = -68.790565  

Logistic regression                                     Number of obs =    100
                                                        Wald chi2(1)  =   1.53
                                                        Prob > chi2   = 0.2168
Log pseudolikelihood = -68.790565                       Pseudo R2     = 0.0073

                                      (Std. err. adjusted for 5 clusters in i)
------------------------------------------------------------------------------
             |               Robust
          d2 | Coefficient  std. err.      z    P>|z|     [95% conf. interval]
-------------+----------------------------------------------------------------
          x1 |   .2125917   .1721239     1.24   0.217    -.1247649    .5499483
       _cons |  -.0654961   .1587345    -0.41   0.680    -.3766099    .2456178
------------------------------------------------------------------------------
...
. * REGRESSION INJECTION START
. if (`repbox_reg_rc' == 0) {
.   parmest, label saving("~/repbox/projects_test/test/repbox/stata/tsv/1_4_`re
> pbox_local_cmd_count'.dta", replace)
...
scalars:
          ...
          e(converged) =  1
                 e(rc) =  0
            e(N_clust) =  5
                 e(ll) =  -68.79056508483097
         e(k_eq_model) =  1
               e(ll_0) =  -69.29471672244782
               e(df_m) =  1
               e(chi2) =  1.525493487446586
                  e(p) =  .2167900539621481
              e(N_cdf) =  0
              e(N_cds) =  0
               e(r2_p) =  .00727547007135

macros:
            e(cmdline) : "logit d2 x1 if probit_sample == 1, vce(cluster i)"
                e(cmd) : "logit"
...
. else {
.   display "#~# REGRESSION COMMAND FAILED 1 4 `repbox_local_cmd_count' rc=`rep
> box_reg_rc'"
. }
...
. local repbox_cmdline = `"dprobit d1 x1"'
...
. capture:  noisily: dprobit d1 x1

Iteration 0:   log likelihood = -69.294717
Iteration 1:   log likelihood = -68.765878
Iteration 2:   log likelihood = -68.765793

Probit regression, reporting marginal effects           Number of obs =    100
                                                        LR chi2(1)    =   1.06
                                                        Prob > chi2   = 0.3037
Log likelihood = -68.765793                             Pseudo R2     = 0.0076

------------------------------------------------------------------------------
      d1 |      dF/dx   Std. err.      z    P>|z|     x-bar  [    95% C.I.   ]
---------+--------------------------------------------------------------------
      x1 |   .0543959    .053137     1.02   0.306   .118868  -.049751  .158543
---------+--------------------------------------------------------------------
  obs. P |        .49
 pred. P |   .4902143  (at x-bar)
------------------------------------------------------------------------------
...
. * REGRESSION INJECTION START
. if (`repbox_reg_rc' == 0) {
.   parmest, label saving("~/repbox/projects_test/test/repbox/stata/tsv/1_5_`re
> pbox_local_cmd_count'.dta", replace)
(file /home/rstudio/repbox/projects_test/test/repbox/stata/tsv/1_5_5.dta not
    found)
file ~/repbox/projects_test/test/repbox/stata/tsv/1_5_5.dta saved
. 
. 
. 
. qui {
.   display "#~# INJECT REG_ERETURN 1 5 `repbox_local_cmd_count'"
#~# INJECT REG_ERETURN 1 5 5
.   ereturn list

scalars:
                  e(N) =  100
               e(ll_0) =  -69.29471672244782
                 e(ll) =  -68.76579264571393
               e(df_m) =  1
               e(chi2) =  1.057848153467774
               e(r2_p) =  .0076329639798144
              e(N_cdf) =  0
              e(N_cds) =  0
               e(pbar) =  .49
               e(xbar) =  -.0245316700339493
             e(offbar) =  0

macros:
                e(cmd) : "dprobit"
            e(cmdline) : "dprobit d1 x1"
              e(dummy) : " 0 0"
              e(title) : "Probit regression"
                e(vce) : "oim"
             e(depvar) : "d1"
...
functions:
             e(sample)   
.   display "#~# END INJECT REG_ERETURN 1 5 `repbox_local_cmd_count'"
#~# END INJECT REG_ERETURN 1 5 5
. }

. else {
.   display "#~# REGRESSION COMMAND FAILED 1 5 `repbox_local_cmd_count' rc=`rep
> box_reg_rc'"
. }
...
. local repbox_cmdline = `"xi: dprobit d1 x1 i.i1 i2"'
...
. capture:  noisily: xi: dprobit d1 x1 i.i1 i2
i.i1              _Ii1_1-4            (naturally coded; _Ii1_1 omitted)

Iteration 0:   log likelihood = -69.294717
Iteration 1:   log likelihood = -66.057206
Iteration 2:   log likelihood = -66.049949
Iteration 3:   log likelihood = -66.049949

Probit regression, reporting marginal effects           Number of obs =    100
                                                        LR chi2(5)    =   6.49
                                                        Prob > chi2   = 0.2615
Log likelihood = -66.049949                             Pseudo R2     = 0.0468

------------------------------------------------------------------------------
      d1 |      dF/dx   Std. err.      z    P>|z|     x-bar  [    95% C.I.   ]
---------+--------------------------------------------------------------------
      x1 |   .0578137   .0537527     1.08   0.282   .118868   -.04754  .163167
  _Ii1_2*|  -.1632153   .1335948    -1.19   0.233       .29  -.425056  .098626
  _Ii1_3*|   .0955377   .1488356     0.64   0.524       .21  -.196175   .38725
  _Ii1_4*|  -.0409815   .1476988    -0.28   0.782       .24  -.330466  .248503
      i2 |  -.0614007   .0438167    -1.40   0.161      2.61   -.14728  .024478
---------+--------------------------------------------------------------------
  obs. P |        .49
 pred. P |   .4880994  (at x-bar)
------------------------------------------------------------------------------
...
. * REGRESSION INJECTION START
. if (`repbox_reg_rc' == 0) {
.   parmest, label saving("~/repbox/projects_test/test/repbox/stata/tsv/1_6_`re
> pbox_local_cmd_count'.dta", replace)
...
scalars:
                  e(N) =  100
               e(ll_0) =  -69.29471672244782
                 e(ll) =  -66.04994929171828
               e(df_m) =  5
               e(chi2) =  6.489534861459077
               e(r2_p) =  .0468256107276704
              e(N_cdf) =  0
              e(N_cds) =  0
               e(pbar) =  .49
               e(xbar) =  -.0298349220225253
             e(offbar) =  0

macros:
                e(cmd) : "dprobit"
            e(cmdline) : "dprobit d1 x1 _Ii1_* i2"
              e(dummy) : " 0 1 1 1 0 0"
              e(title) : "Probit regression"
                e(vce) : "oim"
             e(depvar) : "d1"
...
. else {
.   display "#~# REGRESSION COMMAND FAILED 1 6 `repbox_local_cmd_count' rc=`rep
> box_reg_rc'"
. }
...
```
# END OF FILE: repbox_probit_logit.log
...
I wonder about the issue in runid=5. Can you detect, explain and fix the problem in the code?
Here relevant packages:

All parcel contents for runid=5
...
1     5 sb      x1          x1         ""         d1     0.136  0.133
2     5 sb      (Intercept) /d1        "Constant" d1    -0.0407 0.127
       t     p ci_low ci_up
   <dbl> <dbl>  <dbl> <dbl>
1  1.02  0.306 -0.125 0.398
2 -0.322 0.748 -0.289 0.207
...
# A tibble: 2 × 12
  runid variant cterm       shown_term label      eq       coef    se
  <int> <chr>   <chr>       <chr>      <chr>      <chr>   <dbl> <dbl>
1     5 so      x1          x1         ""         ""     0.136  0.133
2     5 so      (Intercept) _cons      "Constant" ""    -0.0407 0.127
       t     p ci_low ci_up
   <dbl> <dbl>  <dbl> <dbl>
1  1.02  0.306 -0.125 0.398
2 -0.322 0.748 -0.289 0.207
...
1     5 rb      (Intercept) (Intercept) ""    d1    -0.0407 0.127 -0.320
2     5 rb      x1          x1          ""    d1     0.136  0.134  1.02 
      p ci_low ci_up
  <dbl>  <dbl> <dbl>
1 0.749 -0.290 0.209
2 0.307 -0.125 0.398
...
# A tibble: 2 × 17
  runid variant cterm ia_cterm basevar role  var_org_type var_reg_type
  <int> <chr>   <chr> <chr>    <chr>   <chr> <chr>        <chr>       
1     5 sb      d1    d1       d1      dep   dummy        dummy       
2     5 sb      x1    x1       x1      exo   numeric      numeric     
  ia_reg_type absorbed_fe main_pos ia_pos ia_num ia_source_expr
  <chr>       <lgl>          <int>  <int>  <int> <chr>         
1 dummies     FALSE              1      1      1 d1            
2 numeric     FALSE              2      1      1 x1            
