...
Look at the following do file:

```
use "ms_67_4_18_128.dta", clear

reg logdiff myavailratio i.older15

reg logdiff myavailratio i.older15 if year==2012
```

The first regression yields exactly the same coefs in R as in Stata. The 2nd not. The problem for the filtered data year=2012 i.older15 only has value NA and 0 and will be dropped in Stata regression. 

Our R translation already seems to drop i.older15 quite early. The Stata regression has fewer observations than the R translation. I guess the rows where i.older15 is NA will be removed from Stata, but not from R.

Can you discuss whether the Stata behavior can be replicated in our pipeline and how complex it would be. Discuss pro and cons doing it. One pro is clearly that it is nice if our pipeline can exactly replicate Stata's behavior. But if our pipeline gets too complex / runs longer that is also a con.
