...
mrb_test_data_preview_text = function(runid, drf, parcels, n = 5, opts=mrb_test_opts()) {
  ...
  if (opts$show_reg_data) {
    ...
    regvar = parcel_for_runid(parcels$regvar, runid)
    regxvar = if (!is.null(parcels$regxvar)) parcel_for_runid(parcels$regxvar, runid) else tibble()
    dat = try(mrb_get_regression_data(runid, drf, regvar, regxvar), silent = TRUE)
...
mrb_test_opts = function(show_org_data=TRUE, show_reg_data=TRUE, show_pre_reg_data=TRUE, data_head_rows=10, data_tail_rows=0,data_width=1000, max_cases=Inf, just_runid=NULL, ignore_flags=NULL, data_add_org_row=FALSE) {
  as.list(environment())
}
...
```
# END OF FILE: cmdpart_create.R

-----------------------------------------------------------


# FILE: cmdpart_tests.R
```
# FILE: cmdpart_test.R
...
```
# END OF FILE: cmdpart_use.R

-----------------------------------------------------------


# FILE: detach_all_pkg.R
```
example = function() {
  detach_all_packages()
}
...
* This should work and be the only regression in metareg
...
### source_with_log start: 2026-03-13 09:32:51 
...
Error in source_with_log:
package ‘metaregBase’ required by ‘repboxRun’ could not be found

### source_with_log end: 2026-03-13 09:32:51 
In 2 of 4 regressions, problems were detected. 1 problematic cases are shown below.
...
```text
 ...
 (Intercept) -0.1288620 -0.1175014 0.1054785 0.1095908
 L@x1         0.2214535  0.2060900 0.1095375 0.1140649
```
...
```
# A tibble: 30 × 15
          y      x1  `L@x1` .ORG.ROW  year date           i    d1    d2    i1    i2 f1   
      <dbl>   <dbl>   <dbl>    <int> <dbl> <date>     <dbl> <dbl> <dbl> <dbl> <dbl> <chr>
 1 -1.81     1.37   NA             1  2001 2001-01-01     1     1     0     3     3 A    
 2 -0.736    1.18    1.37          2  2002 2002-01-01     1     1     0     2     1 A    
 3  1.19    -0.0386  1.18          3  2003 2003-01-01     1     1     1     4     1 B    
 4 -0.765    2.07   -0.0386        4  2004 2004-01-01     1     1     1     2     3 A    
 5  0.929    0.146   2.07          5  2005 2005-01-01     1     0     0     1     3 B    
 6 -0.399    0.834   0.146         6  2006 2006-01-01     1     0     1     3     3 C    
 7  0.00215 -0.316   0.834         7  2007 2007-01-01     1     1     0     3     1 C    
 8  0.323    0.354  -0.316         8  2008 2008-01-01     1     0     1     1     2 C    
 9 -1.53    -0.451   0.354         9  2009 2009-01-01     1     0     0     1     1 B    
10 -0.180   -1.30   -0.451        10  2010 2010-01-01     1     1     0     3     1 A    
11  0.397    1.52   -1.30         11  2011 2011-01-01     1     0     0     4     1 B    
12  0.656    0.796   1.52         12  2012 2012-01-01     1     0     1     3     2 B    
13  0.213    0.923   0.796        13  2013 2013-01-01     1     0     0     4     4 A    
14  0.518    1.20    0.923        14  2014 2014-01-01     1     0     1     2     1 A    
15 -1.38    -0.559   1.20         15  2015 2015-01-01     1     0     0     4     4 A    
16 -1.66     0.752  -0.559        16  2016 2016-01-01     1     1     1     2     2 A    
17  1.56    -1.00    0.752        17  2017 2017-01-01     1     1     0     3     3 C    
18 -0.364   -0.471  -1.00         18  2018 2018-01-01     1     1     1     4     4 A    
19 -1.34     0.666  -0.471        19  2019 2019-01-01     1     0     1     1     4 C    
20  0.183    0.527   0.666        20  2020 2020-01-01     1     0     0     2     4 B    
21  0.461    0.508   0.527        21  2001 2001-01-01     2     0     0     4     2 C    
22 -1.74    -2.17    0.508        22  2002 2002-01-01     2     1     0     1     2 A    
23 -0.775   -0.740  -2.17         23  2003 2003-01-01     2     1     0     1     3 A    
24 -0.416    0.259  -0.740        24  2004 2004-01-01     2     0     1     4     4 A    
25 -0.843    0.269   0.259        25  2005 2005-01-01     2     0     1     4     3 C    
26 -0.527   -0.214   0.269        26  2006 2006-01-01     2     1     1     3     2 C    
27 -0.437   -0.103  -0.214        27  2007 2007-01-01     2     1     1     1     4 B    
28  2.59    -1.55   -0.103        28  2008 2008-01-01     2     0     1     2     4 A    
29 -0.127   -0.564  -1.55         29  2009 2009-01-01     2     1     1     3     1 C    
30 -1.33     1.24   -0.564        30  2010 2010-01-01     2     1     0     2     3 C    
# ℹ 3 more variables: my_long_factor <chr>, my_long_integer <dbl>, key <dbl>
```
...
My guess for the problem:

The R implementation of the time series operator does currently not account for the panel structure. Observation 21 should be removed, before it is the first period for i=2. 

Is somewhere in my code base above (also old code) handling for panel structure already implemented?

If yes, where?

What should I copy in the new code base, and how to integrate it.
