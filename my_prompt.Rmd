...
For some regressions like areg, xtreg often coefficients only differ in the intercept (see screenshot of a test report). I think it might be sensible to ignore for all Stata commands that use fixed effects a possible intercept in the regcheck comparision between sb and rb. In other words we don't compare intercepts for Stata and R translations, neither value nor existence of intercept.

Discuss and propose the code change. Maybe for the implementation make a helper function that returns a string vector of cmds that are "fixed effects" Stata regression commands for which intercepts will be ignored.
