...
repboxStataReg: Tools features to extract regression information from Stata reproduction run,
  These functions will be called before the metaregBase analysis.
  Also contains utility functions to extract cmdpart format for regressions

repboxUtils: Utility functions. In particular using cmdpart approach.
...
Encoding: UTF-8
LazyData: true
Depends: repboxDB, repboxUtils, repboxDRF, repboxStataReg
```
# END OF FILE: DESCRIPTION
...
#' Process a single regression, expand syntax, and format standard parcels
mrb_run_r_reg_step = function(mrb, runid) {
  restore.point("mrb_run_r_base_step")
  ...
  # To do run regression and extract parcels

  # mainly reg_coef, perhaps also regscalars / regstring using broom
...
mrb_make_r_reg_parcels = function(mrb, save=TRUE) {
  ...
  parcels = list()
  all_step_parcels = mrb$all_r_reg_step_parcels
  if (is.null(all_step_parcels)) {
...
The metaregBase package is currently in a refactoring stage. Next I want to finish the mrb_r_reg.R file. Idea, after we have run the functions in mrb_r_base.R, we have generated the main parcels with regression information from the metaregBase Stata run.

Now we want to use these parcels to translate every regression to R, run it, compare with the Stata results and store a regcoef_r parcel and a regcheck parcel that determines the comparison.

Can you implement this given your info? If yes, Write the modified mrb_r_reg.R in an R code fence and explain. Otherwise, tell me what info you need.
