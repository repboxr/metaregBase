...
Random effects regression seems not yet well handled by the pipeline. Example:


```
# Stata: xi: xtreg difference_belief is_rel_overconf choice escalate practice_score_max period if feedback == 0 & period > 3, re robust
data = stata2r::scmd_keep(data, r_if_cond = "s2r_stata_logical(`feedback` == 0 & `period` > 3)")
...
library(fixest)
rcmd = "feols"
cc_cols = c("is_rel_overconf", "choice", "escalate", "difference_belief", "practice_score_max", "period", "uniqueid")
dat = stata_drop_missing(dat, cc_cols)
formula = `difference_belief` ~ `is_rel_overconf` + `choice` + `escalate` + `practice_score_max` + `period`
ssc = fixest::ssc()
reg_vcov = ~ `uniqueid`
reg = feols(fml = formula,data = dat,vcov = reg_vcov,ssc = ssc,fixef.rm = "none")
ct = broom::tidy(reg,conf.int=TRUE)
glance = broom::glance(reg)

```

Coefficients from R regression differ from Stata results. 

Probably it is good to add "re" in the reg parcel flags variable (comma separated list of flags) and maybe regtranslate can look whether a re flag is set and then translate to random effects...

Please discuss and make a proposal.
