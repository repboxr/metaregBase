...
find_data_run_path = function(pid_row, srun_df, all_pids=NULL) {
  ...
  while (TRUE) {
    first_row = path[1]
    is_explicit = srun_df$cmd[first_row] == "restore"
    is_implicit = isTRUE(srun_df$is_implicit_restore[first_row])
    ...
    if (is_explicit || is_implicit) {
      ...
      if (!is.na(pr_row)) {
        ...
        if (is_explicit) {
          # Drop explicit restore commands since they are just state management
          path = c(new_path, path[-1])
        } else {
          # Keep the implicit restore command since it is a real data command (e.g. regression)
          path = c(new_path, path)
        }
        next
      }
    }
...
Look at the Stata do log below:
...
. 
. tobit i1 x1, ll cluster(i2)
option cluster() not allowed
r(198);

. version 14: tobit i1 x1, ll cluster(i2)

Tobit regression                                    Number of obs     
> =    100
                                                           Uncensored 
> =     74
Limits: Lower =    1                                    Left-censored 
> =     26
...
> =   0.55
                                                    Prob > F          
> = 0.4614
Log pseudolikelihood = -158.35169                   Pseudo R2         
> = 0.0028

                                     (Std. err. adjusted for 4 cluster
> s in i2)
----------------------------------------------------------------------
> --------
             |               Robust
          i1 | Coefficient  std. err.      t    P>|t|     [95% conf. i
> nterval]
-------------+--------------------------------------------------------
> --------
          x1 |   .1447565   .1957855     0.74   0.461    -.2437244    
> .5332375
       _cons |   2.202828    .264649     8.32   0.000     1.677707    
> 2.727949
-------------+--------------------------------------------------------
> --------
      /sigma |   1.439084   .0882328                      1.264011    
> 1.614157
----------------------------------------------------------------------
> --------

. version 14: tobit i1 x1, ll vce(cluster i2)
...
. 
. tobit i1 x1, ll vce(robust)

Refining starting values:

Grid node 0:   log likelihood = -164.22269

Fitting full model:

Iteration 0:   log pseudolikelihood = -164.22269  
Iteration 1:   log pseudolikelihood = -158.57102  
Iteration 2:   log pseudolikelihood = -158.35267  
Iteration 3:   log pseudolikelihood = -158.35169  
Iteration 4:   log pseudolikelihood = -158.35169  
...
-------------+--------------------------------------------------------
> --------
          x1 |   .1447565   .1429428     1.01   0.314     -.138873    
> .4283861
       _cons |   2.202828   .1654962    13.31   0.000     1.874448    
> 2.531208
-------------+--------------------------------------------------------
> --------
    var(e.i1)|   2.070963   .3211951                      1.522377    
> 2.817233
----------------------------------------------------------------------
> --------

. tobit i1 x1, ll robust
option robust not allowed
r(198);

. version 14: tobit i1 x1, ll robust
...
> --------
          x1 |   .1447565   .1429428     1.01   0.314     -.138873    
> .4283861
       _cons |   2.202828   .1654961    13.31   0.000     1.874448    
> 2.531208
-------------+--------------------------------------------------------
> --------
      /sigma |   1.439084    .111597                      1.217651    
> 1.660517
----------------------------------------------------------------------
> --------
...
-------------+--------------------------------------------------------
> --------
          x1 |   .1447565   .1957855     0.74   0.461    -.2437245    
> .5332375
       _cons |   2.202828   .2646491     8.32   0.000     1.677707    
> 2.727949
-------------+--------------------------------------------------------
> --------
    var(e.i1)|   2.070963    .253949                      1.623691    
> 2.641444
----------------------------------------------------------------------
> --------
...
Does not run with Stata from version 15 onwards but works with older Stata. Can you discuss the following plan:

In repboxStata create a new function that detects cmdlines in an older format and stores the older Stata version we need to run it in.

Currently the function shall just detect the old tobit uses (and set version 14 there), but possibly we will add more later. Detection may use stringi regex heuristics but should be fast, e.g. first screen for candidate commands based on cmd == "tobit" condition.

The corresponding cmdlines shall be modified depending on the detected Stata version e.g. to

version 14: tobit ...

This modification shall sustain for later repboxDRF, metaregbase analysis etc, i.e. directly change the cmdline in the relevant parcels.

Does this look sensible? Discuss and if sensible directly also propose the concrete code changes. If there are open issues, ask first.
