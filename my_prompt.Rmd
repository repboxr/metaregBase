...
In an example I have an so / sb mismatch in

qui reg `y' `peervars' `controls', cluster(groep)

suggesting that some data modification command in the pipeline is ommited. Here the do file until this regression (copy pasted from html report). 

The test_pid_1671.do file is just:

```
adopath + "~/ado/plus"
adopath + "/home/rstudio/repbox/projects/restud_84_2_2/drf/ado/_Supplementary"
use "/home/rstudio/repbox/projects/restud_84_2_2/drf/cached_dta/1614_cache.dta", clear
qui reg credits c.mgpa c.sdgpa c.mgpa#c.sdgpa c.gpa#(c.mgpa c.sdgpa c.mgpa#c.sdgpa) ibn.xcats##c.appordr male young old hbo gpa, cluster(groep)
```
...
cls
/******************************************************************************/
// MAIN ANALYSIS SCRIPT FOR THE PAPER
// "Ability peer effects in university: Evidence from a randomized experiment"
// by Booij, Leuven & Oosterbeek
/******************************************************************************/
clear all
//goto Ability project folder
//cap cd "x:\Ability"
//cap cd "/Users/hooster1/Dropbox/Ability/"
/******************************************************************************/
// PROGRAMS
/******************************************************************************/
set scheme s1mono
//ssc install estout, replace
// little helper program to spit out nicely formatted estimates
program _se
args b se df
	local p = 2 * ttail(`df', abs(`b' / `se'))
	di as result %8.3f `b' _c
 	if (`p' < 0.01)      di "***" _c
	else if (`p' < 0.05) di "** " _c
	else if (`p' < 0.10) di "*  " _c
	else               di "   " _c
	local se = "   (" + trim(string(`se', "%8.3f")) + ")"
	di "`se'" _c
end
// calculate *sample* average tracking effects relative to mixing
// mean,sd specification
program ate 
args tr by
preserve
local cons _cons
local spec : colnames e(b)
local spec : list spec - cons
quietly {
	levelsof `by', local(lvlby)
	
	drop mgpa sdgpa
	bysort g1 : egen mgpa  = peer(gpa)
	bysort g1 : egen sdgpa = peer((gpa - mgpa)^2), f(sqrt)
	foreach l in all `lvlby' {
		matrix accum tmp = `spec' if inlist("`l'", string(`by'), "all"), means(mix_`l')
	}
	drop mgpa sdgpa
	bysort `tr' : egen mgpa  = peer(gpa)
 	bysort `tr' : egen sdgpa = peer((gpa - mgpa)^2), f(sqrt)
	foreach l in all `lvlby' {
		matrix accum tmp = `spec' if  inlist("`l'", string(`by'), "all"), means(tr_`l')
		matrix d = tr_`l' - mix_`l'
		matrix e = e(b) * d' , d * e(V) * d'
		noi _se e[1,1] sqrt(e[1,2]) e(N)-e(df_m)
	}
	noi di
}
end
...
/******************************************************************************/
use work, clear
egen xcats = group(wisb gpacat_intake cohort)
label var wisb "Adv. Math"
label def wisb 0 "Reg." 1 "Adv."
label val wisb wisb
// FIGURE 1
preserve
	collapse mgpa sdgpa, by(groep)
	set obs 400 //redundant obs for rendering circles
	tempvar x y1 y2 y12 y22 y3
	gen `x'   = .70 * (_n - 1) / (_N - 1)  - .35
	gen `y1'  =  sqrt((1 - (`x' / 0.25)^2)) * 0.18 + 1
	gen `y2'  = -sqrt((1 - (`x' / 0.25)^2)) * 0.18 + 1
	gen `y12' =  sqrt((1 - (`x' / 0.35)^2)) * 0.25 + 1
	gen `y22' = -sqrt((1 - (`x' / 0.35)^2)) * 0.25 + 1
	gen `y3'  = 1
	twoway  ///
	(line `y1' `y2' `y12' `y22' `x', color(gray gray gray gray) lp(solid solid dash dash) ) ///
	(scatter `y3' `x' if `x'==0    , color(gray) msymbol(O) msize(medium) ) ///
	(sc sdgpa mgpa, mc(bmrblue) ms(O) msize(medium)) ///
		, xtitle(mean GPA of peers) ytitle(SD GPA of peers) ///
		xlabel(-1(.5)1.5) ylabel(.4(.2)1.4, angle(0)) scheme(s1mono) ///
		legend(r(1) label(6 "Experimental")  label(1 "Natural 95%") label(3 "Natural 99%") order(6 3 1)) name(scatter)
	//gr export fig/scatter-vargpacircles.pdf, replace
	gr export fig/fig1.eps, replace
restore
// TABLE 1, FIGURE 2, and TABLE A2
// run analysis_priordists to get these
//TEACHERS
preserve
qui {
use data\wteachers, clear
noi mean tchr*
//Teacher char analyses
eststo senpr_r :  reg tchr_prof *gpa*  i.cohort, r
repbox outputrunid 453
eststo senpr   :  reg tchr_prof *gpa*  i.cohort
repbox outputrunid 454
eststo senph_r :  reg tchr_ph *gpa*  i.cohort, r
repbox outputrunid 455
eststo senph   :  reg tchr_ph *gpa*  i.cohort
repbox outputrunid 456
eststo male_r :  reg tchr_male *gpa* i.cohort, r
repbox outputrunid 457
eststo male   :  reg tchr_male *gpa* i.cohort
repbox outputrunid 458
noi di _n _n "Semester 1 courses Micro, Wis, V&M: 3 * (14 + 17 + 17 groups in est sample) = 144 observations"
noi di  "PLUS 17 from MSTRAT(2011) (was OenO in 09 and 10 but those not available, teacher left UvA and lost lists (personal communication))"
noi esttab *_r, keep(*gpa*)  b(%6.2f) se nogaps s(r2 N) obslast staraux star(* 0.10 ** 0.05 *** 0.01)
suest senpr senph male
noi testparm *gpa*
}
restore
// TABLE 2
fvexpand  male age hbo gparaw i.gpacat_intake wisb appordr mgpa sdgpa crd1raw cijfer_av_prop dropout
noi di as text _col(23) 2009 _skip(10) 2010 _skip(10) 2011
qui foreach v in `r(varlist)' {
	// we want means for all factors incl the baseline, so remove b.
	local v = subinstr("`v'", "b.", ".", .)
	noi di as txt "`v'" _col(20) _c
	forv c=2009/2011 {
		qui sum `v' if cohort == `c'
		noi di as res %7.2f r(mean) _c 
		local sd = "(" + trim(strofreal(r(sd), "%6.2f")) + ")"
		if !inlist(`v',0 ,1) noi di %7s "`sd'" _c
		else noi di "       " _c
	}
	noi di
}
qui bys cohort groep (id): gen Ngroups = groep if _n==1
qui gen Nstudents =id
tabstat Ngroups Nstudents, by(cohort) s(count)
 drop N*
// TABLE 3
unab   xcontrols : male young old hbo gpa
fvunab rcontrols : ibn.xcats##c.appordr 
local ccredits cluster(groep)
local cavgrade cluster(groep)
local cdropout cluster(groep)
est clear
qui foreach v of var mgpa sdgpa credits avgrade dropout {
	reg `v' `xcontrols' `rcontrols', r `c`v''
repbox outputrunid 647, 655, 663, 671, 679
	sum `v'
	estadd scalar my  = r(mean)
	estadd scalar sdy = r(sd)
	testparm `xcontrols'
	estadd scalar chi2 = r(df) * r(F)
	estadd scalar pval = r(p)
	est store `v'
}
esttab *, keep(`xcontrols')  obslast sca(my sdy pval r2) b(3) se star(* .1 ** .05 *** .01) nogaps
qui reg mgpa `xcontrols' `rcontrols'
repbox outputrunid 688
est store mgpa
qui reg sdgpa `xcontrols' `rcontrols'
repbox outputrunid 690
est store sdgpa
qui suest mgpa sdgpa
testparm `xcontrols'
// TABLE 4
//recenter treatvariables for ease of interpretation
preserve
est clear
sum sdgpa
g mean_sdgpa = r(mean)
replace sdgpa = sdgpa - r(mean)
sum mgpa
g mean_mgpa = r(mean)
replace mgpa = mgpa - r(mean)
local spec0 c.mgpa
local spec1 `spec0'
local spec2 `spec1' c.sdgpa
local spec3 `spec2' c.mgpa#c.sdgpa
local spec4 `spec3' c.gpa#(`spec3')
local x0 `rcontrols'
local x1 `x0' `xcontrols'
local x2 `x1'
local x3 `x1'
local x4 `x1'
local y credits
qui forv s=0/4 {
	reg `y' `spec`s'' `x`s'', cluster(groep)
repbox outputrunid 714, 718, 722, 726, 730
	testparm `spec`s''
	estadd scalar p = r(p)
	est store s`s'
}
esttab s*, keep(*mgpa* *sdgpa* gpa) order(mgpa sdgpa c.mgpa#c.sdgpa gpa) stats(p) b(3) se star(* .1 ** .05 *** .01) nogaps
noi testparm c.gpa#(`spec3')
// TABLE A3
qui foreach y of var credits avgrade dropout {
	foreach s in 3 4 {
		reg `y' `spec`s'' `x`s'', cluster(groep)
repbox outputrunid 737, 746, 755, 764, 773, 782
		testparm `spec`s''
		estadd scalar p = r(p)
		capture testparm c.sdgpa c.mgpa#c.sdgpa c.gpa#(`spec3')
		if (_rc!=0)	capture testparm c.sdgpa c.mgpa#c.sdgpa
		capture estadd scalar plin = r(p)
		capture testparm c.gpa#(`spec3')
		capture estadd scalar ph = r(p)
		estadd ysumm
		est store _`y'`s'
	}
}
esttab _*, keep(*mgpa* *sdgpa* gpa) order(mgpa sdgpa c.mgpa#c.sdgpa gpa) sfmt(%6.3f %6.3f %6.0f %6.0f %6.3f %6.3f %6.3f) noobs sca(ymean ysd N N_clust p plin ph) b(3) se star(* .1 ** .05 *** .01) nogaps
restore
// FIGURE 3
//Effect mGPA by sdGPA
qui {
qui eststo main: reg credits (c.sdgpa c.mgpa c.sdgpa#c.mgpa)##c.gpa  i.xcat##c.appordr male old young hbo , cluster(groep)
repbox outputrunid 794
qui margins, dydx(mgpa) at(sdgpa=(0.3(0.1)1.5)) level(90)
marginsplot,  ///
	yline(0)  ///
	plotopt(ms(O) lp(dash) mc(black) ) ///
	recastci(rarea) ciopt(fcolor(bluishgray) lcolor(bluishgray)) ///
	ytitle(Marginal effect mean GPA of peers) ///
	xtitle(SD GPA of peers) ///
	name(mgpa2x, replace) title("") ///
	addplot((function y=0, ra(0.29 1.51) ylabel(-1(0.5)1, angle(0)) xlabel(0.3 0.4 0.5 "{bf:0.5}" 0.6(.1)1 1.1 "{bf:1.1}" 1.2(.1)1.5) lc(black)  sort yline(0, lc(black)))) legend(off)
//gr export fig/dydx_mgpa_x_sdgpa.pdf, replace
gr export fig/fig3a.eps, replace
//Effect sdGPA by mGPA
qui margins, dydx(sdgpa) at(mgpa=(-1(0.2)1.6)) level(90) vsquish
marginsplot,  ///
	yline(0)  ///
	plotopt(ms(O)  lp(dash) mc(black) ) ///
	recastci(rarea) ciopt(fcolor(bluishgray) lcolor(bluishgray)) ///
	ytitle(Marginal effect SD GPA of peers) ///
	xtitle(Mean GPA of peers) ///
	name(sdgpa2x, replace) title("") ///
	addplot((function y=0, ra(-1.01 1.61) ylabel(-1(0.5)1, angle(0)) xlabel(-1 -.8 -.6 "{bf:-.6}" -.4(.2).4 .6 "{bf:.6}" .8(.2)1.6) lc(black)  sort yline(0, lc(black)))) legend(off)
//gr export fig/dydx_sdgpa_x_mgpa.pdf, replace
gr export fig/fig3b.eps, replace
//Effect mGPA by myGPA over homo vs hetero groups
est restore main
margins, dydx(mgpa) at(gpa=(-1.5(0.5)1.5) sdgpa=(0.50 1.14)) level(90)
margins, dydx(mgpa) at(gpa=(-2(0.5)2) sdgpa=(0.50 1.14)) level(90)
mat res = r(table)
mat at = r(at)
mat at = at[.,"sdgpa"], at[.,"gpa"], res[1 ,1...]', res[5..6, 1...]'
cap drop at*
svmat at
preserve
keep at*
...
drop if x==.
drop at1
reshape wide at3 at4 at5, i(at2) j(x)
se2plot at31 at41 at51 at32 at42 at52 at2, ///
	copt(lpat(dash ..) ms(O Oh) mc(black ..)) ///
	xtitle(Own GPA) ylabel(, angle(0)) ytitle(Marginal effect mean GPA of peers) ///
	xlabel(-2(0.5)2) legend(order(1 "Homogeneous groups" 3 "Heterogeneous groups")) ///
	legend(region(lcolor(white))) ylabel(-1(0.5)1) name(m1, replace)
restore
//gr export fig/dydx_mgpa_x_gpa.pdf, replace
gr export fig/fig3c.eps, replace
//Effect sdGPA by myGPA over Low v.s. High ability groups
est restore main
margins, dydx(sdgpa) at(gpa=(-1.5(0.5)1.5) mgpa=(-0.60 0.65)) level(90)
margins, dydx(sdgpa) at(gpa=(-2(0.5)2) mgpa=(-0.60 0.65)) level(90)
mat res = r(table)
mat at = r(at)
mat at = at[.,"mgpa"], at[.,"gpa"], res[1 ,1...]', res[5..6, 1...]'
cap drop at*
svmat at
preserve
keep at*
...
se2plot at31 at41 at51 at32 at42 at52 at2, ///
	copt(lpat(dash ..) ms(O Oh) mc(black ..)) ///
	xtitle(Own GPA) ylabel(, angle(0)) ytitle(Marginal effect SD GPA of peers) ///
	xlabel(-2(0.5)2) legend(order(1 "Low ability groups" 3 "High ability groups")) ///
	legend(region(lcolor(white))) ylabel(-1(0.5)1) name(sd1, replace)
restore
//gr export fig/dydx_sdgpa_x_gpa.pdf, replace
gr export fig/fig3d.eps, replace
}
// TABLE 5 & 6
//use if treat==1 using work, clear
//egen xcats = group(wisb gpacat_intake cohort)
gen g1  = 1                                      // mix
set seed 11032016
g rx = runiform()
sort gpa rx
foreach c in 2 3 {
	gen g`c'a = floor(`c'*(_n-1)/_N) + 1
}
recode g3a (1 3 = 1)       , gen(g3m)            // bifurcate
recode g3a (1 2 = 1) (3=2) , gen(g3h)            // mix two bottom groups
recode g3a (2 3 = 2)       , gen(g3l)            // mix two top groups
program track_effects
	di as txt "2-way tracking" _col(15) _c
	ate g2 g2
	di as txt "3-way tracking" _col(15) _c
	ate g3a g3a
	di as txt "track low"      _col(15) _c
	ate g3l g3a
	di as txt "track middle"   _col(15) _c
	ate g3m g3a
	di as txt "track high"     _col(15) _c
	ate g3h g3a
end
foreach y of var credits avgrade dropout {
	qui reg `y' c.gpa##(c.mgpa c.sdgpa c.mgpa#c.sdgpa) ibn.xcats##c.appordr male young old hbo gpa, cluster(groep)
```

Here an excerpt of stata_run_cmd for runids between cache and regression:

```
> stata_run_cmd %>% filter(runid >= 1614 & runid <= 1671) %>% as.data.frame()
   runid             root_file_path                  file_path counter line      cmd
1   1614 _Supplementary/analysis.do _Supplementary/analysis.do    1186   31     egen
2   1615 _Supplementary/analysis.do _Supplementary/analysis.do    1187   33   matrix
3   1616 _Supplementary/analysis.do _Supplementary/analysis.do    1188   34   matrix
4   1617 _Supplementary/analysis.do _Supplementary/analysis.do    1189   35   matrix
5   1618 _Supplementary/analysis.do _Supplementary/analysis.do    1190   36      _se
6   1619 _Supplementary/analysis.do _Supplementary/analysis.do    1191    5     args
7   1620 _Supplementary/analysis.do _Supplementary/analysis.do    1192    6    local
8   1621 _Supplementary/analysis.do _Supplementary/analysis.do    1193    7       di
9   1622 _Supplementary/analysis.do _Supplementary/analysis.do    1194   12    local
10  1623 _Supplementary/analysis.do _Supplementary/analysis.do    1195   13       di
11  1624 _Supplementary/analysis.do _Supplementary/analysis.do    1196   33   matrix
12  1625 _Supplementary/analysis.do _Supplementary/analysis.do    1197   34   matrix
13  1626 _Supplementary/analysis.do _Supplementary/analysis.do    1198   35   matrix
14  1627 _Supplementary/analysis.do _Supplementary/analysis.do    1199   36      _se
15  1628 _Supplementary/analysis.do _Supplementary/analysis.do    1200    5     args
16  1629 _Supplementary/analysis.do _Supplementary/analysis.do    1201    6    local
17  1630 _Supplementary/analysis.do _Supplementary/analysis.do    1202    7       di
18  1631 _Supplementary/analysis.do _Supplementary/analysis.do    1203   12    local
19  1632 _Supplementary/analysis.do _Supplementary/analysis.do    1204   13       di
20  1633 _Supplementary/analysis.do _Supplementary/analysis.do    1205   33   matrix
21  1634 _Supplementary/analysis.do _Supplementary/analysis.do    1206   34   matrix
22  1635 _Supplementary/analysis.do _Supplementary/analysis.do    1207   35   matrix
23  1636 _Supplementary/analysis.do _Supplementary/analysis.do    1208   36      _se
24  1637 _Supplementary/analysis.do _Supplementary/analysis.do    1209    5     args
25  1638 _Supplementary/analysis.do _Supplementary/analysis.do    1210    6    local
26  1639 _Supplementary/analysis.do _Supplementary/analysis.do    1211    7       di
27  1640 _Supplementary/analysis.do _Supplementary/analysis.do    1212   12    local
28  1641 _Supplementary/analysis.do _Supplementary/analysis.do    1213   13       di
29  1642 _Supplementary/analysis.do _Supplementary/analysis.do    1214   33   matrix
30  1643 _Supplementary/analysis.do _Supplementary/analysis.do    1215   34   matrix
31  1644 _Supplementary/analysis.do _Supplementary/analysis.do    1216   35   matrix
32  1645 _Supplementary/analysis.do _Supplementary/analysis.do    1217   36      _se
33  1646 _Supplementary/analysis.do _Supplementary/analysis.do    1218    5     args
34  1647 _Supplementary/analysis.do _Supplementary/analysis.do    1219    6    local
35  1648 _Supplementary/analysis.do _Supplementary/analysis.do    1220    7       di
36  1649 _Supplementary/analysis.do _Supplementary/analysis.do    1221   12    local
37  1650 _Supplementary/analysis.do _Supplementary/analysis.do    1222   13       di
38  1651 _Supplementary/analysis.do _Supplementary/analysis.do    1223   38       di
39  1652 _Supplementary/analysis.do _Supplementary/analysis.do    1224  229  tabstat
40  1653 _Supplementary/analysis.do _Supplementary/analysis.do    1225  231 preserve
41  1654 _Supplementary/analysis.do _Supplementary/analysis.do    1226  232 collapse
42  1655 _Supplementary/analysis.do _Supplementary/analysis.do    1227  233     sort
43  1656 _Supplementary/analysis.do _Supplementary/analysis.do    1228  234      gen
44  1657 _Supplementary/analysis.do _Supplementary/analysis.do    1229  235  reshape
45  1658 _Supplementary/analysis.do _Supplementary/analysis.do    1230  236     egen
46  1659 _Supplementary/analysis.do _Supplementary/analysis.do    1231  237     egen
47  1660 _Supplementary/analysis.do _Supplementary/analysis.do    1232  238     forv
48  1661 _Supplementary/analysis.do _Supplementary/analysis.do    1233  239      gen
49  1662 _Supplementary/analysis.do _Supplementary/analysis.do    1234  239      gen
50  1663 _Supplementary/analysis.do _Supplementary/analysis.do    1235  239      gen
51  1664 _Supplementary/analysis.do _Supplementary/analysis.do    1236  241   twoway
52  1665 _Supplementary/analysis.do _Supplementary/analysis.do    1237  242    graph
53  1666 _Supplementary/analysis.do _Supplementary/analysis.do    1238  243  restore
54  1667 _Supplementary/analysis.do _Supplementary/analysis.do    1239  244    local
55  1668 _Supplementary/analysis.do _Supplementary/analysis.do    1240  245    local
56  1669 _Supplementary/analysis.do _Supplementary/analysis.do    1241  246    local
57  1670 _Supplementary/analysis.do _Supplementary/analysis.do    1242  247    local
58  1671 _Supplementary/analysis.do _Supplementary/analysis.do    1243  254      reg
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               cmdline
1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              bysort g3h : egen sdgpa = peer((gpa - mgpa)^2), f(sqrt)
2  matrix accum tmp = gpa mgpa sdgpa c.mgpa#c.sdgpa c.gpa#c.mgpa c.gpa#c.sdgpa c.gpa#c.mgpa#c.sdgpa 1bn.xcats 2.xcats 3.xcats 4.xcats 5.xcats 6.xcats 7.xcats 8.xcats 9.xcats 10.xcats 11.xcats 12.xcats 13.xcats 14.xcats 15.xcats 16.xcats 17.xcats 18o.xcats appordr 1bn.xcats#c.appordr 2.xcats#c.appordr 3.xcats#c.appordr 4.xcats#c.appordr 5.xcats#c.appordr 6.xcats#c.appordr 7.xcats#c.appordr 8.xcats#c.appordr 9.xcats#c.appordr 10.xcats#c.appordr 11.xcats#c.appordr 12.xcats#c.appordr 13.xcats#c.appordr 14.xcats#c.appordr 15.xcats#c.appordr 16.xcats#c.appordr 17.xcats#c.appordr 18o.xcats#co.appordr male young old hbo o.gpa if  inlist("all", string(g3a), "all"), means(tr_all)
3                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          matrix d = tr_all - mix_all
4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 matrix e = e(b) * d' , d * e(V) * d'
5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             noi _se e[1,1] sqrt(e[1,2]) e(N)-e(df_m)
6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         args b se df
7                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        local p = 2 * ttail(e(N)-e(df_m), abs(e[1,1] / sqrt(e[1,2])))
8                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         di as result %8.3f e[1,1] _c
9                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        local se = "   (" + trim(string(sqrt(e[1,2]), "%8.3f")) + ")"
10                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  di "   (0.017)" _c
11     matrix accum tmp = gpa mgpa sdgpa c.mgpa#c.sdgpa c.gpa#c.mgpa c.gpa#c.sdgpa c.gpa#c.mgpa#c.sdgpa 1bn.xcats 2.xcats 3.xcats 4.xcats 5.xcats 6.xcats 7.xcats 8.xcats 9.xcats 10.xcats 11.xcats 12.xcats 13.xcats 14.xcats 15.xcats 16.xcats 17.xcats 18o.xcats appordr 1bn.xcats#c.appordr 2.xcats#c.appordr 3.xcats#c.appordr 4.xcats#c.appordr 5.xcats#c.appordr 6.xcats#c.appordr 7.xcats#c.appordr 8.xcats#c.appordr 9.xcats#c.appordr 10.xcats#c.appordr 11.xcats#c.appordr 12.xcats#c.appordr 13.xcats#c.appordr 14.xcats#c.appordr 15.xcats#c.appordr 16.xcats#c.appordr 17.xcats#c.appordr 18o.xcats#co.appordr male young old hbo o.gpa if  inlist("1", string(g3a), "all"), means(tr_1)
12                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             matrix d = tr_1 - mix_1
13                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                matrix e = e(b) * d' , d * e(V) * d'
14                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            noi _se e[1,1] sqrt(e[1,2]) e(N)-e(df_m)
15                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        args b se df
16                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       local p = 2 * ttail(e(N)-e(df_m), abs(e[1,1] / sqrt(e[1,2])))
17                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        di as result %8.3f e[1,1] _c
18                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       local se = "   (" + trim(string(sqrt(e[1,2]), "%8.3f")) + ")"
19                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  di "   (0.025)" _c
20     matrix accum tmp = gpa mgpa sdgpa c.mgpa#c.sdgpa c.gpa#c.mgpa c.gpa#c.sdgpa c.gpa#c.mgpa#c.sdgpa 1bn.xcats 2.xcats 3.xcats 4.xcats 5.xcats 6.xcats 7.xcats 8.xcats 9.xcats 10.xcats 11.xcats 12.xcats 13.xcats 14.xcats 15.xcats 16.xcats 17.xcats 18o.xcats appordr 1bn.xcats#c.appordr 2.xcats#c.appordr 3.xcats#c.appordr 4.xcats#c.appordr 5.xcats#c.appordr 6.xcats#c.appordr 7.xcats#c.appordr 8.xcats#c.appordr 9.xcats#c.appordr 10.xcats#c.appordr 11.xcats#c.appordr 12.xcats#c.appordr 13.xcats#c.appordr 14.xcats#c.appordr 15.xcats#c.appordr 16.xcats#c.appordr 17.xcats#c.appordr 18o.xcats#co.appordr male young old hbo o.gpa if  inlist("2", string(g3a), "all"), means(tr_2)
21                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             matrix d = tr_2 - mix_2
22                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                matrix e = e(b) * d' , d * e(V) * d'
23                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            noi _se e[1,1] sqrt(e[1,2]) e(N)-e(df_m)
24                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        args b se df
25                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       local p = 2 * ttail(e(N)-e(df_m), abs(e[1,1] / sqrt(e[1,2])))
26                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        di as result %8.3f e[1,1] _c
27                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       local se = "   (" + trim(string(sqrt(e[1,2]), "%8.3f")) + ")"
28                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  di "   (0.022)" _c
29     matrix accum tmp = gpa mgpa sdgpa c.mgpa#c.sdgpa c.gpa#c.mgpa c.gpa#c.sdgpa c.gpa#c.mgpa#c.sdgpa 1bn.xcats 2.xcats 3.xcats 4.xcats 5.xcats 6.xcats 7.xcats 8.xcats 9.xcats 10.xcats 11.xcats 12.xcats 13.xcats 14.xcats 15.xcats 16.xcats 17.xcats 18o.xcats appordr 1bn.xcats#c.appordr 2.xcats#c.appordr 3.xcats#c.appordr 4.xcats#c.appordr 5.xcats#c.appordr 6.xcats#c.appordr 7.xcats#c.appordr 8.xcats#c.appordr 9.xcats#c.appordr 10.xcats#c.appordr 11.xcats#c.appordr 12.xcats#c.appordr 13.xcats#c.appordr 14.xcats#c.appordr 15.xcats#c.appordr 16.xcats#c.appordr 17.xcats#c.appordr 18o.xcats#co.appordr male young old hbo o.gpa if  inlist("3", string(g3a), "all"), means(tr_3)
30                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             matrix d = tr_3 - mix_3
31                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                matrix e = e(b) * d' , d * e(V) * d'
32                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            noi _se e[1,1] sqrt(e[1,2]) e(N)-e(df_m)
33                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        args b se df
34                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       local p = 2 * ttail(e(N)-e(df_m), abs(e[1,1] / sqrt(e[1,2])))
35                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        di as result %8.3f e[1,1] _c
36                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       local se = "   (" + trim(string(sqrt(e[1,2]), "%8.3f")) + ")"
37                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  di "   (0.029)" _c
38                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              noi di
39                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        tabstat dropout, by(g3a) s(mean sd) f(%6.3f)
40                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            preserve
41                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    collapse mgpa ngrrel*, by(groep)
42                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           sort mgpa
43                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  gen mg3 =(_n/_N>1/3) + (_n/_N>2/3)
44                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              reshape long ngrrel, i(groep) j(month)
45                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              bys month: egen mngrrel = mean(ngrrel)
46                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       bys mg3 month:  egen mg3ngrrel = mean(ngrrel)
47                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    forv i = 0 / 2 {
48                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  gen mg3_0 =  mg3ngrrel if   mg3==0
49                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  gen mg3_1 =  mg3ngrrel if   mg3==1
50                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  gen mg3_2 =  mg3ngrrel if   mg3==2
51                                                                                                                                                                                                                                                                                                                                                                       twoway (line mg3_0 mngrrel  mg3_2   month, sort lp(solid dash solid) lc(bmrblue black dkred)  text( 0.73 9 "Average")  text( 0.87 9 "High GPA group")  text( 0.54 9 "Low GPA group")  ytitle("Groupsize as fraction of initial") ylabel(0(0.2)1,  angle(h)) xlabel(1(1)10) xtitle("Month"))  , name(ngr, replace) legend(off)
52                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  graph export fig/fig4.eps, replace
53                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             restore
54                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          local controls ibn.xcats##c.appordr male young old hbo gpa
55                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  local peervars c.mgpa c.sdgpa c.mgpa#c.sdgpa c.gpa#(c.mgpa c.sdgpa c.mgpa#c.sdgpa)
56                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          local peeriv   c.amgpa c.asdgpa c.amgpa#c.asdgpa c.gpa#(c.amgpa c.asdgpa c.amgpa#c.asdgpa)
57                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     local y credits
58                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     qui reg credits c.mgpa c.sdgpa c.mgpa#c.sdgpa c.gpa#(c.mgpa c.sdgpa c.mgpa#c.sdgpa) ibn.xcats##c.appordr male young old hbo gpa, cluster(groep)
   missing_data found_path start_time end_time errcode   out_img_file
1         FALSE              12:18:32 12:18:32       0               
2         FALSE              12:18:32 12:18:32       0               
3         FALSE              12:18:32 12:18:32       0               
4         FALSE              12:18:32 12:18:32       0               
5         FALSE              12:18:32 12:18:32       0               
6         FALSE              12:18:32 12:18:32       0               
7         FALSE              12:18:32 12:18:32       0               
8         FALSE              12:18:32 12:18:32       0               
9         FALSE              12:18:32 12:18:32       0               
10        FALSE              12:18:32 12:18:32       0               
11        FALSE              12:18:32 12:18:32       0               
12        FALSE              12:18:32 12:18:32       0               
13        FALSE              12:18:32 12:18:32       0               
14        FALSE              12:18:32 12:18:32       0               
15        FALSE              12:18:32 12:18:32       0               
16        FALSE              12:18:32 12:18:32       0               
17        FALSE              12:18:32 12:18:32       0               
18        FALSE              12:18:32 12:18:32       0               
19        FALSE              12:18:32 12:18:32       0               
20        FALSE              12:18:32 12:18:32       0               
21        FALSE              12:18:32 12:18:32       0               
22        FALSE              12:18:32 12:18:32       0               
23        FALSE              12:18:32 12:18:32       0               
24        FALSE              12:18:32 12:18:32       0               
25        FALSE              12:18:32 12:18:32       0               
26        FALSE              12:18:32 12:18:32       0               
27        FALSE              12:18:32 12:18:32       0               
28        FALSE              12:18:32 12:18:32       0               
29        FALSE              12:18:32 12:18:32       0               
30        FALSE              12:18:32 12:18:32       0               
31        FALSE              12:18:32 12:18:32       0               
32        FALSE              12:18:32 12:18:32       0               
33        FALSE              12:18:32 12:18:32       0               
34        FALSE              12:18:32 12:18:32       0               
35        FALSE              12:18:32 12:18:32       0               
36        FALSE              12:18:32 12:18:32       0               
37        FALSE              12:18:32 12:18:32       0               
38        FALSE              12:18:32 12:18:32       0               
39        FALSE              12:18:32 12:18:32       0               
40        FALSE              12:18:32 12:18:32       0               
41        FALSE              12:18:32 12:18:32       0               
42        FALSE              12:18:32 12:18:32       0               
43        FALSE              12:18:32 12:18:32       0               
44        FALSE              12:18:32 12:18:32       0               
45        FALSE              12:18:32 12:18:32       0               
46        FALSE              12:18:32 12:18:32       0               
47        FALSE              12:18:32     <NA>      NA               
48        FALSE              12:18:32 12:18:32       0               
49        FALSE              12:18:32 12:18:32       0               
50        FALSE              12:18:32 12:18:32       0               
51        FALSE              12:18:32 12:18:32       0 3_241_1236.svg
52        FALSE   fig4.eps   12:18:32 12:18:32       0 3_242_1237.svg
53        FALSE              12:18:32 12:18:32       0               
54        FALSE              12:18:32 12:18:32       0               
55        FALSE              12:18:32 12:18:32       0               
56        FALSE              12:18:32 12:18:32       0               
57        FALSE              12:18:32 12:18:32       0               
58        FALSE              12:18:32 12:18:32       0    
```

Any idea what goes wrong an idea for a bug fix. Explain and propose the fix.
