...
In an example I have a problem with the following regression (runid=2501)

qui eststo iv2: ivregress 2sls credits (c.mgpa c.sdgpa c.mgpa#c.sdgpa c.gpa#(c.mgpa c.sdgpa c.mgpa#c.sdgpa) ngrav = noshow c.amgpa c.asdgpa c.amgpa#c.asdgpa c.gpa#(c.amgpa c.asdgpa c.amgpa#c.asdgpa)) ibn.xcats##c.appordr male young old hbo gpa, cluster(groep)

Below is a transcript of some of my tests / explorations inside mrb_run_r_base_step. Can you detect the problem, describe it and provide a fix? I am not sure at which level the bug occurs, already at the cmdpart parser in repboxUtils or later in metaregBase. Try to solve it at the route but keep the code performant, the cmdpart parser seems already quite slow sometimes...

```

>   mrb = mrb_run_r_base(mrb,just_pids=pid,continue_on_error = FALSE)

mrb_r_base processing runids: 2501 
The regression variable = could not be matched with any variable in the data set.

The regression variable appordr male young old hbo gpa could not be matched with any variable in the data set.

The regression variable appordr male young old hbo gpa could not be matched with any variable in the data set.

The regression variable appordr male young old hbo gpa could not be matched with any variable in the data set.
 Error: Base variable  does not exist in data set and no xi-generated source column could be found for cterm = 
6.
stop(msg, call. = FALSE) at problems.R#110
5.
repbox_problem(type = "regvar_no_match", msg = msg, fail_action = "error") at mrb_cterms.R#352
4.
create_cterm_col(dat, cterm, timevar = timevar, panelvar = panelvar, 
    tdelta = tdelta) at mrb_cterms.R#275
3.
create_cterm_cols(dat_full, ct_cterms_tmp, timevar = xtvar$timevar, 
    panelvar = xtvar$panelvar, tdelta = xtvar$tdelta) at mrb_r_base.R#76
2.
mrb_run_r_base_step(mrb, pid, continue_on_error = continue_on_error) at mrb_r_base.R#350
1.
mrb_run_r_base(mrb, just_pids = pid, continue_on_error = FALSE) 

>   restore.point("mrb_run_r_base_step")

3: mrb_run_r_base(mrb, just_pids = pid, continue_on_error = FALSE)
2: mrb_run_r_base_step(mrb, pid, continue_on_error = continue_on_error)
1: restore.point("mrb_run_r_base_step")Restored:  continue_on_error,mrb,pid,with_try
>   if (with_try) {
+     restore.point("mrb_run_r_base_step_with_try")
+     res = repboxUtils::try_catch_repbox_problems(mrb_run_r_base_step(mrb,pid, with_try=FALSE, continue_on_error = continue_on_error),project_dir = mrb$project_dir,runid = pid,msg_prefix = "mrb_run_r_base_step: ", err_val=NULL)
+     return(res$value)
...
> cmdpart
# A tibble: 31 × 6
   runid parent    part    counter content                                                          tag  
   <int> <chr>     <chr>     <dbl> <chr>                                                            <chr>
 1  2501 ""        main          0 {{pre1}} {{pre2}} {{cmd}} {{subcmd}} {{varlist}},{{opt_str1}}    ""   
 2  2501 "main"    pre           1 qui                                                              "qui…
 3  2501 "main"    pre           2 eststo iv2:                                                      ""   
 4  2501 "main"    cmd           0 ivregress                                                        "ivr…
 5  2501 "main"    varlist       0 {{v1}} {{v2}} {{v3}} {{v4}} {{v5}} {{v6}} {{v7}} {{v8}} {{v9}} … ""   
 6  2501 "main"    subcmd        0 2sls                                                             "2sl…
 7  2501 "varlist" v             1 credits                                                          "dep…
 8  2501 "varlist" v             2 ngrav                                                            "exo"
 9  2501 "varlist" v             3 =                                                                "exo"
10  2501 "varlist" v             4 noshow                                                           "exo"
# ℹ 21 more rows
...
> se_info
# A tibble: 1 × 3
  se_category se_type se_args       
  <chr>       <chr>   <chr>         
1 cluster     cluster cluster1=groep
>   # 3. Create TS columns on UNFILTERED data
>   # We build a lightweight, temporary regvar just to discover which cterms need evaluating.
>   regvar_tmp = cmdpart_to_regvar(cmdpart, dat_full, opts_df, se_info)
>   regvar_tmp = mrb_add_xtreg_fe_regvar(regvar_tmp, run_obj, opts_df, xtvar, dat_full)
...
[10] "xcats"                                               
[11] "groep"                                               
[12] "appordr male young old hbo gpa"                      
[13] "(c.mgpac.sdgpac.mgpa"                                
[14] "amgpa)ibn.xcats"                                     
[15] "asdgpa)ibn.xcats"                                    
[16] "appordrmaleyoungoldhbogpa"                           
[17] "(c.mgpac.sdgpac.mgpa#gpa#sdgpa"                      
[18] "mgpa#sdgpa"                                          
[19] "amgpa#asdgpa"                                        
[20] "amgpa)ibn.xcats#appordrmaleyoungoldhbogpa#gpa"       
[21] "appordrmaleyoungoldhbogpa#asdgpa)ibn.xcats#gpa"      
[22] "amgpa#appordrmaleyoungoldhbogpa#asdgpa)ibn.xcats#gpa"
> run_obj$cmdline
[1] "qui eststo iv2: ivregress 2sls credits (c.mgpa c.sdgpa c.mgpa#c.sdgpa c.gpa#(c.mgpa c.sdgpa c.mgpa#c.sdgpa) ngrav = noshow c.amgpa c.asdgpa c.amgpa#c.asdgpa c.gpa#(c.amgpa c.asdgpa c.amgpa#c.asdgpa)) ibn.xcats##c.appordr male young old hbo gpa, cluster(groep)"
