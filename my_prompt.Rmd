...
I would like to store information of quasi regs and post regs

quasi regs are Stata commands like `rd` that are similar to regression analysis but will not be handled by our full mrb pipeline since output is different from normal regressions. 

post regs are post regression commands, like tests associated with regressions.

I created two new parcel descriptions:

```
name: quasi_reg

descr: |
  Basic for quasi regression commands.

unique_index:
  main_index: [runid, variant]
...
fields:
  ...
  flags:
    descr: |
      Possibly flags that contain some info

  has_err:
    type: bool
    descr: did the stata command run without error in the original replication
```
...
descr: |
  Basic information for post-regression commands.
...
fields:
  ...
  reg_runid:
    type: int
    descr: The runid of the associated regression command
...
I would like to adapt our mrb pipeline so that these parcels are generated. They provide only very little infos but can still be helpful for more customized meta studies that analyse these quasi regressions or some post regressions.

Can you implement the parcel generation?

1. For post_reg I want a fast functions that determines the reg_runid based on our run_df. This function could also be part of repbox_drf. Perhaps we may even generate a complete drf_post_reg.R file that has more post_reg utilities, like generating a drf object for post_reg commands that allows to build a path_df for post reg commands. But by default we don't want to generate such paths.

2. I was thinking whether for quasi regs we should also store timevar, panelvar and tdelta info, as we do for regressions. This would require likely modification of repboxStata or repboxStataReg. But that might be helpful. What do you think?

Please discuss, make a code proposal and explain.
