...
probit d1 x1 c.x#i.i1, vce(robust)
...
> g
  log type:  text
 opened on:  23 Apr 2026, 13:58:52
.   adopath + "~/ado/plus"
...
. local repbox_cmdline = `"probit d1 x1 c.x#i.i1, vce(robust)"'
...
. capture:  noisily: probit d1 x1 c.x#i.i1, vce(robust)
...
macros:
            e(cmdline) : "probit d1 x1 c.x#i.i1, vce(robust)"
                e(cmd) : "probit"
...
. local repbox_cmdline = `"logit d2 x1 x if probit_sample == 1, vce(cluster i)"
> '
...
. capture:  noisily: logit d2 x1 x if probit_sample == 1, vce(cluster i)
...
macros:
            e(cmdline) : "logit d2 x1 x if probit_sample == 1, vce(cluster .."
                e(cmd) : "logit"
...
. local repbox_cmdline = `"dprobit d1 x1 x i.i1 i.i2"'
...
. capture:  noisily: dprobit d1 x1 x i.i1 i.i2
...
. 
. * REGRESSION INJECTION START
. parmest, label saving("~/repbox/projects_test/test/repbox/stata/tsv/1_5_`repb
> ox_local_cmd_count'.dta", replace)
...
The probit / logit pipeline does not yet work nicely. Below are also resulting parcels for an example runid. Any glue what is the problem, how to fix it? Also look at the Stata documention for probit / logit output.

Discuss and explain and provide a solution proposal.
...
1     5 sb      x1    x1         ""    ""    0.0531 0.0430  1.24 0.216 -0.0311 0.137
...
# A tibble: 3 × 12
  runid variant cterm       shown_term label      eq       coef     se      t      p ci_low ci_up
  <int> <chr>   <chr>       <chr>      <chr>      <chr>   <dbl>  <dbl>  <dbl>  <dbl>  <dbl> <dbl>
1     5 so      x1          x1         ""         d2     0.213   0.172  1.24   0.217 -0.125 0.550
2     5 so      x1          o.x1       ""         d2    NA      NA     NA     NA      0     0    
3     5 so      (Intercept) /d2        "Constant" d2    -0.0655  0.159 -0.413  0.680 -0.377 0.246
...
# A tibble: 2 × 12
  runid variant cterm       shown_term  label eq       coef    se      t     p ci_low ci_up
  <int> <chr>   <chr>       <chr>       <chr> <chr>   <dbl> <dbl>  <dbl> <dbl>  <dbl> <dbl>
1     5 rb      (Intercept) (Intercept) ""    ""    -0.0407 0.127 -0.320 0.749 -0.290 0.209
2     5 rb      x1          x1          ""    ""     0.136  0.134  1.02  0.307 -0.125 0.398

Parcel regvar:

# A tibble: 5 × 17
  runid variant cterm ia_cterm basevar role  var_org_type var_reg_type ia_reg_type absorbed_fe
  <int> <chr>   <chr> <chr>    <chr>   <chr> <chr>        <chr>        <chr>       <lgl>      
1     5 sb      i1    i1       i1      exo   integer      factor       factor      FALSE      
2     5 sb      i2    i2       i2      exo   integer      factor       factor      FALSE      
3     5 sb      d1    d1       d1      dep   dummy        dummy        dummies     FALSE      
4     5 sb      x1    x1       x1      exo   numeric      numeric      numeric     FALSE      
5     5 sb      x1    x1       x1      exo   numeric      numeric      numeric     FALSE      
  main_pos ia_pos ia_num ia_source_expr var_source_expr transform transform_par
     <int>  <int>  <int> <chr>          <chr>           <chr>     <chr>        
1        4      1      1 i.i1           i.i1            "i"       "1"          
2        5      1      1 i.i2           i.i2            "i"       "1"          
3        1      1      1 d1             d1              ""        ""           
4        2      1      2 x1             x1              ""        ""           
5        3      2      2 x1             x1              ""        ""           
...
# A tibble: 10 × 6
 ...
 1     5 i1       i1=3  exo   FALSE       NA     
 2     5 i1       i1=2  exo   FALSE       NA     
 3     5 i1       i1=4  exo   FALSE       NA     
 4     5 i1       i1=1  exo   FALSE       NA     
 5     5 i2       i2=3  exo   FALSE       NA     
 6     5 i2       i2=1  exo   FALSE       NA     
 7     5 i2       i2=2  exo   FALSE       NA     
 8     5 i2       i2=4  exo   FALSE       NA     
 9     5 x1       x1    exo   TRUE         0.0531
10     5 x1       x1#x1 exo   FALSE       NA     
