...
### source_with_log start: 2026-03-24 07:48:50 
...
mrb_r_base processing runids: 3 5 12 18 19 27 28 31 Error in `data[, all_vars, drop = FALSE]` at stata2r/R/t_estimation_cmd.R:158:5:
! Can't subset columns that don't exist.
✖ Column `i2#year` doesn't exist.
...
27: (function (cnd) 
    {
        ...
        cnd_signal(cnd)
    })(list(message = "", trace = list(call = list(metaregBase::mrb_run_as_test(project_dir = "~/repbox/projects/test", 
        "~/repbox/projects/test/run/run_test.R", opts = opts), source_with_log(run_script, 
        log_con = con), withCallingHandlers(source(file = file, local = envir, 
        echo = echo, print.eval = print.eval, prompt.echo = prompt.echo, 
        max.deparse.length = max.deparse.length, chdir = chdir, keep.source = keep.source), 
        warning = function(w) {
            warnings_seen <<- c(warnings_seen, conditionMessage(w))
            invokeRestart("muffleWarning")
        }), source(file = file, local = envir, echo = echo, print.eval = print.eval, 
        prompt.echo = prompt.echo, max.deparse.length = max.deparse.length, 
        chdir = chdir, keep.source = keep.source), withVisible(eval(ei, 
        envir)), eval(ei, envir), eval(ei, envir), mrb_run_r_base(mrb), 
        mrb_run_r_base_step(mrb, pid), repboxDRF::drf_get_data(pid, 
            drf = mrb$drf), eval(rcode_call, envir = exec_env), eval(rcode_call, 
            envir = exec_env), scmd_estimation_effects(data = data, 
            dep_var = "y", model_vars = c("x1", "i1", "i2#year"), 
            needed_e = c("e(N)", "e(r2)"), estimator = "reghdfe", 
            formula_terms = c("x1")), stats::complete.cases(data[, 
            all_vars, drop = FALSE]), data[, all_vars, drop = FALSE], 
        `[.tbl_df`(data, , all_vars, drop = FALSE), vectbl_as_col_location(j, 
        ...
        abort(class = c(class, "vctrs_error_subscript"), i = i, ..., 
            call = call)), parent = c(0L, 1L, 2L, 2L, 4L, 4L, 6L, 
    1L, 8L, 9L, 10L, 11L, 12L, 13L, 13L, 13L, 16L, 17L, 18L, 17L, 
    0L, 21L, 22L, 23L), visible = c(TRUE, TRUE, TRUE, TRUE, TRUE, 
    TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, 
    FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE), namespace = c("metaregBase", 
    "metaregBase", "base", "base", "base", "base", "base", "metaregBase", 
    "metaregBase", "repboxDRF", "base", "base", "stata2r", "stats", 
    ...
    FALSE, FALSE, FALSE, FALSE, FALSE)), parent = NULL, i = c("y", 
    "x1", "i1", "i2#year"), subscript_type = "character", names = c("year", 
    "date", "i", "y", "x1", "d1", "d2", "i1", "i2", "f1", "my_long_factor", 
    "my_long_integer", "key", "stata2r_original_order_idx"), subscript_action = NULL, 
        subscript_arg = "j", rlang = list(inherit = TRUE), call = data[, 
            all_vars, drop = FALSE]))
...
21: (function () 
    stop_subscript_oob(i = i, subscript_type = subscript_type, names = names, 
        subscript_action = subscript_action, subscript_arg = subscript_arg, 
        call = call))()
20: vec_as_location(j, n, names, missing = "error", call = call)
19: withCallingHandlers(expr, vctrs_error_subscript = function(cnd) {
        cnd$subscript_arg <- j_arg
        cnd$subscript_elt <- "column"
        if (isTRUE(assign) && !isTRUE(cnd$subscript_action %in% c("negate"))) {
            cnd$subscript_action <- "assign"
...
18: subclass_col_index_errors(vec_as_location(j, n, names, missing = "error", 
        call = call), j_arg = j_arg, assign = assign)
17: vectbl_as_col_location(j, length(x), names(x), j_arg = j_arg, 
        assign = FALSE)
16: `[.tbl_df`(data, , all_vars, drop = FALSE) at t_estimation_cmd.R#158
15: data[, all_vars, drop = FALSE] at t_estimation_cmd.R#158
14: stats::complete.cases(data[, all_vars, drop = FALSE]) at t_estimation_cmd.R#158
13: scmd_estimation_effects(data = data, dep_var = "y", model_vars = c("x1", 
        "i1", "i2#year"), needed_e = c("e(N)", "e(r2)"), estimator = "reghdfe", 
        formula_terms = c("x1")) at <text>#5
...
8: mrb_run_r_base(mrb) at run_test.R#32
7: eval(ei, envir)
6: eval(ei, envir)
