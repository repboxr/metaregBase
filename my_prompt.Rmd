...
I recently implemented the creation of the quasi_reg and post_reg parcels. quasi_reg shall also have timevar, panelvar and tdelta like reg parcel. Yet, that is not correctly implemented. Here an error:

3 of the 2503 run commands without missing data threw an error.
Error in `dplyr::transmute()`:
ℹ In argument: `timevar = as.character(repboxUtils::na.val(timevar, ""))`.
Caused by error in `repboxUtils::na.val()`:
! object 'timevar' not found
...
19: .handleSimpleError(function (cnd) 
    ...
    }, "object 'timevar' not found", base::quote(repboxUtils::na.val(timevar, 
        ""))) at tools.R#408
18: repboxUtils::na.val(timevar, "")
17: .Call(dplyr_mask_eval_all_mutate, quo, private)
16: eval()
15: mask$eval_all_mutate(quo)
14: mutate_col(dots[[i]], data, mask, new_columns)
13: withCallingHandlers(for (i in seq_along(dots)) {
...
12: mutate_cols(.data, dots, by)
11: transmute.data.frame(., runid = as.integer(runid), variant = "so", 
        cmd = as.character(cmd), cmdline = as.character(cmdline), 
        flags = "", has_err = errcode != 0, timevar = as.character(repboxUtils::na.val(timevar, 
            "")), panelvar = as.character(repboxUtils::na.val(panelvar, 
            "")), tdelta = as.character(repboxUtils::na.val(tdelta, 
            "")))
10: dplyr::transmute(., runid = as.integer(runid), variant = "so", 
        cmd = as.character(cmd), cmdline = as.character(cmdline), 
        flags = "", has_err = errcode != 0, timevar = as.character(repboxUtils::na.val(timevar, 
            "")), panelvar = as.character(repboxUtils::na.val(panelvar, 
            "")), tdelta = as.character(repboxUtils::na.val(tdelta, 
            "")))
9: run_df[qrows, , drop = FALSE] %>% dplyr::transmute(runid = as.integer(runid), 
       variant = "so", cmd = as.character(cmd), cmdline = as.character(cmdline), 
       flags = "", has_err = errcode != 0, timevar = as.character(repboxUtils::na.val(timevar, 
           "")), panelvar = as.character(repboxUtils::na.val(panelvar, 
           "")), tdelta = as.character(repboxUtils::na.val(tdelta, 
           ""))) at rsr.R#179
8: rsr_make_quasi_and_post_reg_parcels(project_dir, parcels = parcels, 
       run_df = run_df) at rsr.R#29
7: rsr_make_reg_info(project_dir = project_dir, overwrite = overwrite, 
       parcels = parcels, repbox_results = repbox_results)
6: rb_postprocess_stata_reproduction(rb = rb, overwrite = overwrite) at run_post_gha.R#37
5: run_all_post_gha() at run_post_gha.R#52
4: eval(ei, envir)
3: eval(ei, envir)
2: withVisible(eval(ei, envir))
1: source("~/repbox/projects/aejapp_10_4_6/run/run_post_gha.R")


I think repboxStata or repboxStataReg probably needs to write those variables in the original reproduction run, as is done for regressions. They then end up in the xtvar parcel and also need to be obtained from them. But that pipeline is not yet correctly implemented for quasi_reg. (post_reg is fine, we don't store timevar etc for that parcel).

Analyse, propose a fix and explain.
