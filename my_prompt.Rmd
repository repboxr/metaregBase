...
Three things to improve the report:

1. I have regcheck parcels for which regcoef_so has coefs but regcoef (i.e. sb run) has no coefs, even though sb was run. This typically has a lot of NA in regcheck (see screenshot). Currently, these regressions are just mentioned as "rb missing" in the report, but they should get a better title, e.g. "sb coefs missing, no rb".


2. In the code pane, can you have three differnet colors:

- regressions that were run fine without any isse (dark green as before)

- regressions that have an issue (red)

- regressions that did not run at all (also so missing) e.g. due to missing data (violet)

3. The folder "{project_dir}/drf/cached_dta/" has files named {runid}_cache.dta. These are custom cache files. Can you mark in the code panel script lines where we have a cache? But in a way that takes not more general space for all code lines. Maybe just add for those code lines that have a cache a simple unicode symbol (U+1F4C1) &#128193 for file folder. Other lines should stay as they are, no new general column.
