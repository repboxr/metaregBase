...
The different ib prefixes like ibn. ib(first). etc are not yet treated by our pipeline. See example script and test log below. Can we easily implement them? In the end it seems mainly about the reference level. Do we already try to determine the reference level based on the Stata output or can we store somewhere the reference level in our parcel? Or would need to augment some parcel structure. Explain and make a sensible proposal (less changes would be preferable).
...
reg y ib(last).i1
reg y ib(freq).i1
reg y ib1.i1 // reference is i1=1
...
```
Replicate  test  with  1  do files.

1 of 1 inject and run

Run  /home/rstudio/repbox/projects_test/test/mod/code/repbox_reg_ia.do
    finished after 0.19 seconds.


Run  /home/rstudio/repbox/projects_test/test/metareg/base/stata_code/mrb_stata.do
    finished after 0.18 seconds.

mrb_r_base processing runids: 2 3 
mrb_run_r_base_step: Error: Column ibn.i1 does not exist in data set and thus I cannot generate the cterm ibn.i1

4 
mrb_run_r_base_step: Error: Column ib(first).i1 does not exist in data set and thus I cannot generate the cterm ib(first).i1

5 
mrb_run_r_base_step: Error: Column ib(last).i1 does not exist in data set and thus I cannot generate the cterm ib(last).i1

6 
mrb_run_r_base_step: Error: Column ib(freq).i1 does not exist in data set and thus I cannot generate the cterm ib(freq).i1
...
mrb_r_reg processing runids: 2 3 4 5 6 7 8 9 
```
