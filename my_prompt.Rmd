...
- Very Important: Do not warp code into `try` statements to generally catch errors.
  ...
  this cleanly solves the underlying problem and does not hide other possible problems.f
...
Somehow the the cterm generation for xi generated variables starting with _I is mixed up. I have a test example do file below:
...
The regcoef_so and regcoef_sb parcel has:

> regcoef_so
# A tibble: 10 × 12
   runid variant cterm      shown_term label eq        coef    se       t     p ci_low ci_up
   <int> <chr>   <chr>      <chr>      <chr> <chr>    <dbl> <dbl>   <dbl> <dbl>  <dbl> <dbl>
 5    10 so      x1         x1         ""    ""    -0.00338 0.148 -0.0228 0.982 -0.299 0.293
 6    10 so      x1=-1@874… _Ix1_2     "x1=… ""     1.70    1.16   1.46   0.148 -0.616 4.01 
 7    10 so      (Intercep… _cons      "Con… ""    -0.126   0.130 -0.965  0.338 -0.385 0.134


i.e. we have _Ix1_2 renamed as x1=-1@874…   This corresponds to the label variable that was set in the dta data set once we cached the data file

Yet the regcoef_rb parcel (metaregBase reproduction run) has:

> regcoef_rb
# A tibble: 9 × 12
  runid variant cterm       shown_term  label eq        coef    se       t     p ci_low ci_up
  <int> <chr>   <chr>       <chr>       <chr> <chr>    <dbl> <dbl>   <dbl> <dbl>  <dbl> <dbl>
4    10 rb      (Intercept) (Intercept) ""    ""    -0.126   0.130 -0.965  0.338 -0.385 0.134
5    10 rb      _Ix1_2      `_Ix1_2`    ""    ""     1.70    1.16   1.46   0.148 -0.616 4.01 
6    10 rb      x1          x1          ""    ""    -0.00338 0.148 -0.0228 0.982 -0.299 0.293

So here the cterm for _Ix1_2 is not renamed and the label is "". We use a cache for runid==1 that is loaded in the R reproduction. But the cached dta still has the label for _Ix1_2 (see screenshot).

Finally, after the repair run regvar parcel for runid=10 is:

> regvar
# A tibble: 12 × 17
   runid variant cterm  ia_cterm basevar role  var_org_type var_reg_type ia_reg_type absorbed_fe main_pos
   <int> <chr>   <chr>  <chr>    <chr>   <chr> <chr>        <chr>        <chr>       <lgl>       7    10 sb      _Ix1_2 _Ix1_2   _Ix1_2  exo   dummy        dummy        dummies     FALSE              3
 8    10 sb      y      y        y       dep   numeric      numeric      numeric     FALSE              1
 9    10 sb      x1     x1       x1      exo   numeric      numeric      numeric     FALSE              2
         2
# ℹ 6 more variables: ia_pos <int>, ia_num <int>, ia_source_expr <chr>, var_source_expr <chr>,

So here also the cterm is just _Ix1_2.

We need consistent naming to later match the coefficients from the different regression modes. Can you detect where, how and why the label got lost in the R reproduction and regvar after repair. How could we fix it?
