...
```
# END OF FILE: mrb_test_mod_code.R
...
if (FALSE) {
  metaregBase::mrb_run_as_test(project_dir = "~/repbox/projects/test","~/repbox/projects/test/run/run_test.R")
  rstudioapi::filesPaneNavigate(project_dir)
}
...
### source_with_log start: 2026-03-13 06:23:27 
...
mrb_r_base processing runids: 3 4 5 6 
Error in source_with_log:


### source_with_log end: 2026-03-13 06:23:28 

# Comparision of Stata and R regressions


In 3 of 4 regressions the R and Stata coefficients don't match. 3 problematic cases are shown below.
...
```text
 ...
 x1          -0.09180998 -0.40067661 0.1244126 0.1813916
 (Intercept) -0.10941876 -0.03963154 0.1125622 0.2082003
```
...
```text
 cterm coef_rb    coef_sb    se_rb     se_sb    
 i1=2  -0.2323702 -0.3388709 0.4044543 0.4097375
 i1=3  -0.5087099 -0.6808181 0.4278631 0.4409138
```
...
rcmd = "feols"
dat[["D.__.y"]] = dat[["D@y"]]
formula = `D.__.y` ~ `i1=3` + `i1=2` + `i1=4` + `D@x1`
reg_vcov = "hetero"
reg = feols(fml = formula,data = dat,vcov = reg_vcov)
ct = broom::tidy(reg,conf.int=TRUE)
glance = broom::glance(reg)
...
Can you please rewrite the debug pipeline 
