...
I need to implement proper checks, error logs and possibility to repair failed metaregBase reproductions for particular regressions.

Look at the current code and first give a verbal proposal how to best account for the following points. Later you can also suggest concrete code changes.

- The metaregBase reproduction can fail in different steps, e.g. Stata reproduction or R reproductio, for particular regressions. Currently an error is thrown or just results don't map. I want the option that errors can be captured and the failure is documented. In the end I want to have a parcel regcheck with the following fields:
...
fields:
  ...
  so_did_run:
    type: bool
    descr: Did the original Stata reproduction of the regression run without error

  sb_did_run:
    type: bool
    descr: Did the metareg base Stata replication of the regression run without error
...
So if we have an error say in the R translation of a regression with runid=23. Then the entry for that regression in regcheck shall have reg_ok=FALSE and rb_did_run=FALSE and problem containing a short error message.

- Possibly everything runs through but coefficients differ. Also this shall be proberly stored in the regcheck parcel.

- We may perform "repair runs" of certain runids. E.g. R translation may yield different regression results fail because the translated xi command in R assigs the dummy variables differently than Stata. If a regression translation fails, we may re-run Stata code but generate a file cache directly of the filtered data set used by the regression and then adapt pathes in drf and try the R reproduction again. Since we now have a different data set, a lot of packages might change, i.e. even the regvar, regxvar parcels etc. But only for the subset of regressions, we have changed. I.e. I want to elegantly include functionality to just adapt the parcels that changed when we re-run a subset of runids. Think deeply how to best implement without too much code duplication, following the existing style.
