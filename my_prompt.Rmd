...
```
# Report of test run for project /home/rstudio/repbox/projects/aejpol_2_3_6
...
# Run log of the test script

# Stata Reproduction Run Errors

The following 112 Stata commands threw an error during the original reproduction run:

**runid 6** (File: model-mpg-gasprice-sales.dta)

- **Error Code:** 601
```stata
use model-mpg-gasprice-sales
```
...
- **Error Code:** 111
```stata
gen dlnsales = lnsales-lnsales[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen ddpm = dpm-dpm[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen dum7883 = mdlyear>=1978 & mdlyear<=1983
```
...
- **Error Code:** 111
```stata
gen dum8489 = mdlyear>=1984 & mdlyear<=1989
```
...
- **Error Code:** 111
```stata
gen dum9095 = mdlyear>=1990 & mdlyear<=1994
```
...
- **Error Code:** 111
```stata
gen dum9601 = mdlyear>=1996 & mdlyear<=2001
```
...
- **Error Code:** 111
```stata
gen dum0207 = mdlyear>=2002 & mdlyear<=2007
```
...
- **Error Code:** 111
```stata
gen ddpmdum7883 = dpmdum7883-dpmdum7883[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen ddpmdum8489 = dpmdum8489-dpmdum8489[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen ddpmdum9095 = dpmdum9095-dpmdum9095[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen ddpmdum9601 = dpmdum9601-dpmdum9601[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen ddpmdum0207 = dpmdum0207-dpmdum0207[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
by year month: egen sumsamp = sum(sales*obs)
```
...
- **Error Code:** 111
```stata
gen predsh = ln(sampsh) + [#1]ddpm*deldpm
```
...
- **Error Code:** 111
```stata
by year month: egen sumpred = sum(predsh*obs)
```
...
- **Error Code:** 111
```stata
egen actmpg = sum(sampsh*mpg) if year==2007 & month==8
```
...
- **Error Code:** 111
```stata
egen taxmpg = sum(taxsh*mpg) if year==2007 & month==8
```
...
- **Error Code:** 111
```stata
summ sampsh taxsh actmpg taxmpg if year==2007 & month==8
```
...
- **Error Code:** 111
```stata
by year month: egen sumdpmtax = sum((dpm+(1/mpg))*taxsh) if year==2007 & month==8
```
...
- **Error Code:** 111
```stata
by year month: egen sumdpm = sum(dpm*sampsh) if year==2007 & month==8
```
...
- **Error Code:** 111
```stata
gen gprime = 1-taxmpg*sumdpmtax + actmpg*sumdpm
```
...
- **Error Code:** 111
```stata
gen taxse = sqrt(alphav*gprime*gprime)
```
...
- **Error Code:** 111
```stata
drop sumsamp sampsh deldpm predsh sumpred taxsh actmpg taxmpg alphav sumdpmtax sumdpm gprime taxse
```
...
- **Error Code:** 111
```stata
drop if year<2001 | (year==2001 & month<=8) | year>2007 | (year==2007 & month>=9)
```
...
- **Error Code:** 111
```stata
by mdl mdlyear: egen shasobs = sum(hasobs)
```
...
- **Error Code:** 111
```stata
egen mdl = group(company brand series mdlyear)
```
...
- **Error Code:** 111
```stata
keep mdl year month ym dlnsales ddpm mpg company brand series seasadjprice aggclass sales
```
...
- **Error Code:** 111
```stata
quietly for Y in num 1/1000: gen ddpm_Y = ddpm*(mdl==Y)
```
...
- **Error Code:** 121
```stata
quietly for Y in num 1001/.: gen ddpm_Y = ddpm*(mdl==Y)
```
...
- **Error Code:** 303
```stata
quietly for Y in num 1/1000: replace mdlcoeff = [#1]ddpm_Y if mdl==Y
```
...
- **Error Code:** 121
```stata
quietly for Y in num 1001/.: replace mdlcoeff = [#1]ddpm_Y if mdl==Y
```
...
- **Error Code:** 111
```stata
replace mdlcoeff = mdlcoeff[_n-1] if mdl==mdl[_n-1] & mdlcoeff==.
```
...
- **Error Code:** 111
```stata
replace num = num[_n-1]+1 if mdl==mdl[_n-1]
```
...
- **Error Code:** 111
```stata
egen sumsamp = sum(sales*obs) if num==1
```
...
- **Error Code:** 111
```stata
gen predsh = ln(sampsh) + mdlcoeff*deldpm
```
...
- **Error Code:** 111
```stata
egen sumpred = sum(predsh*obs) if num==1
```
...
- **Error Code:** 111
```stata
summ sampsh taxsh actmpg taxmpg
```
...
- **Error Code:** 111
```stata
gen delh = taxsh*(seasadjprice+1) - taxmpg*taxsh*(seasadjprice+1)/mpg - sampsh*seasadjprice + actmpg*sampsh*seasadjprice/mpg if num==1
```
...
- **Error Code:** 111
```stata
replace delh = 0 if delh==. | num>1 | obs==0
```
...
- **Error Code:** 111
```stata
summ taxmpg taxmpgse
```

**runid 129** (File: model-mpg-gasprice-sales.dta)

- **Error Code:** 601
```stata
use model-mpg-gasprice-sales, clear
```
...
- **Error Code:** 111
```stata
gen dfc6dpm = fc6dpm-fc6dpm[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
gen dactdpm = actdpm-actdpm[_n-1] if mdl==mdl[_n-1] & ((year==year[_n-1] & month==month[_n-1]+1 & month!=9) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12))
```
...
- **Error Code:** 111
```stata
for X in num 20 40 60 80: by year month: egen pXmpg = pctile(mpg) if lnsales!=. &  dpm!=. & mpg!=0 & mpg!=., p(X)
```
...
- **Error Code:** 111
```stata
gen mpgq = 1 if mpg<p20mpg & lnsales!=. &  dpm!=. & mpg!=0 & mpg!=.
```
...
- **Error Code:** 111
```stata
replace mpgq = 2 if mpg>=p20mpg & mpg<p40mpg & lnsales!=. &  dpm!=. & mpg!=0 & mpg!=.
```
...
- **Error Code:** 111
```stata
replace mpgq = 3 if mpg>=p40mpg & mpg<p60mpg & lnsales!=. &  dpm!=. & mpg!=0 & mpg!=.
```
...
- **Error Code:** 111
```stata
replace mpgq = 4 if mpg>=p60mpg & mpg<p80mpg & lnsales!=. &  dpm!=. & mpg!=0 & mpg!=.
```
...
- **Error Code:** 111
```stata
replace mpgq = 5 if mpg>=p80mpg & mpg!=. & lnsales!=. &  dpm!=. & mpg!=0 & mpg!=.
```
...
- **Error Code:** 111
```stata
qui for Y in num 2/12: for Z in num 2/5: gen byte _ImonXmpgq_Y_Z = month==Y & mpgq==Z
```
...
- **Error Code:** 111
```stata
gen ymc = ym*10 + aggclass
```




In 1 of 1 regressions, problems were detected. 1 cases are shown below.

The report uses the fresh `regcheck` parcel as the primary source for flags. `raw_did_run` means the underlying Stata run produced raw results, while `did_run` means the corresponding repdb parcels were also generated.

Problem types and runids:

  - Stata base differs from Stata original.; sb vs so: not identical: 123

## runid 123

**Issues detected:** Stata base differs from Stata original., regcheck reports that sb and so are not identical.
...
**sb vs so difference:**
- No differing coefficients or standard errors to show.

### Original Stata Source

- File: `final-estimation.do`
- Original line: 236

### Code Path
```r
# Stata: use mpg-gasprice, clear
data = drf_load_data(project_dir, "org_data/mpg-gasprice.dta")
data$stata2r_original_order_idx = seq_len(nrow(data))
assign("has_original_order_idx", TRUE, envir = stata2r::stata2r_env)

# Stata: drop if year<2001 | (year==2001 & month<=8) | year>2007 | (year==2007 & month>=9)
data = scmd_drop(data = data, r_if_cond = "`year`<2001 | (`year`==2001 & `month`<=8) | `year`>2007 | (`year`==2007 & `month`>=9)")

# Stata: gen lnprice = ln(seasadjprice)
data = scmd_generate(data = data, new_var = "lnprice", r_expr_str = "`ln`(`seasadjprice`)", is_string = FALSE, force_integer = FALSE)

# Stata: gen lnavmpg = ln(mnmpg)
data = scmd_generate(data = data, new_var = "lnavmpg", r_expr_str = "`ln`(`mnmpg`)", is_string = FALSE, force_integer = FALSE)

# Stata: sort year month
data = scmd_sort(data = data, varlist_str = "year month", type = "sort")

# Stata: gen dlnprice = lnprice - lnprice[_n-1] if (year==year[_n-1] & month==month[_n-1] + 1) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12)
data = scmd_generate(data = data, new_var = "dlnprice", r_expr_str = "`lnprice` - dplyr::lag(`lnprice`, n = 1)", r_if_cond = "(`year`==dplyr::lag(`year`, n = 1) & `month`==dplyr::lag(`month`, n = 1) + 1) | (`year`==dplyr::lag(`year`, n = 1) + 1 & `month`==1 & dplyr::lag(`month`, n = 1)==12)", is_string = FALSE, force_integer = FALSE)

# Stata: gen dlnavmpg = lnavmpg - lnavmpg[_n-1] if (year==year[_n-1] & month==month[_n-1] + 1) | (year==year[_n-1] + 1 & month==1 & month[_n-1]==12)
data = scmd_generate(data = data, new_var = "dlnavmpg", r_expr_str = "`lnavmpg` - dplyr::lag(`lnavmpg`, n = 1)", r_if_cond = "(`year`==dplyr::lag(`year`, n = 1) & `month`==dplyr::lag(`month`, n = 1) + 1) | (`year`==dplyr::lag(`year`, n = 1) + 1 & `month`==1 & dplyr::lag(`month`, n = 1)==12)", is_string = FALSE, force_integer = FALSE)

# Stata: xi i.month
data = scmd_xi(data = data, var1 = "month", noomit = FALSE)

# Stata: areg dlnavmpg dlnprice _Imonth*, absorb(year) robust
runid = 123
if (!exists("parcels")) parcels = list()
parcels = parcels
drf = repboxDRF::drf_load(project_dir, parcels = parcels)
...
formula = `dlnavmpg` ~ `month=2` + `month=3` + `month=4` + `month=5` + `month=6` + `month=7` + `month=8` + `month=9` + `month=10` + `month=11` + `month=12` + `dlnprice` | `year`
ssc = fixest::ssc()
reg_vcov = "hetero"
...
1. Why is only one regression run? Missing data for all others? Can you conclude from the info given to you?

2. Why the difference between sb and so run in regcheck? Coefficients are similar but not identical. Does data set differ? Any idea what is going on?
