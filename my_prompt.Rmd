...
I have a Stata regression:

. xi: dprobit response minority monitoredinfo hs_emailinfo uni_emailinfo  ll_fem ll_unk
> nown apt_size_h apt_price_t apt_equip if minority==0, robust

note: ll_unknown != 0 predicts success perfectly;
      ll_unknown omitted and 7 obs not used.

note: minority omitted because of collinearity.
Iteration 0:   log pseudolikelihood = -334.90251
Iteration 1:   log pseudolikelihood = -326.39262
Iteration 2:   log pseudolikelihood = -326.35112
Iteration 3:   log pseudolikelihood = -326.35112

Probit regression, reporting marginal effects           Number of obs =    599
                                                        Wald chi2(7)  =  20.77
                                                        Prob > chi2   = 0.0041
Log pseudolikelihood = -326.35112                       Pseudo R2     = 0.0255

------------------------------------------------------------------------------
         |               Robust
response |      dF/dx   Std. err.      z    P>|z|     x-bar  [    95% C.I.   ]
---------+--------------------------------------------------------------------
monito~o*|  -.0635034   .0454557    -1.41   0.159    .42571  -.152595  .025588
hs_ema~o*|  -.0006963   .0555879    -0.01   0.990    .18197  -.109647  .108254
uni_em~o*|   .0112351   .0579995     0.19   0.848    .15192  -.102442  .124912
  ll_fem*|   .0316174   .0352352     0.90   0.371   .464107  -.037442  .100677
apt_si~h |   .1771462   .1300969     1.36   0.173   .465843  -.077839  .432131
apt_pr~t |   .0151708   .0068478     2.22   0.027   9.03568   .001749  .028592
apt_eq~p*|  -.0548332   .0524045    -1.08   0.278   .155259  -.157544  .047878
---------+--------------------------------------------------------------------
  obs. P |   .7529215
 pred. P |   .7596624  (at x-bar)
------------------------------------------------------------------------------
(*) dF/dx is for discrete change of dummy variable from 0 to 1
    z and P>|z| correspond to the test of the underlying coefficient being 0

Here is an automatic R translation:

> library(fixest)
> rcmd = "feglm"
> cc_cols = c("response", "minority", "monitoredinfo", "hs_emailinfo", "uni_emailinfo", "ll_fem", "ll_unknown", "apt_equip", "apt_size_h", "apt_price_t")
> cc_cols = intersect(cc_cols, colnames(dat))
> dat = dat[complete.cases(dat[, cc_cols, drop=FALSE]), ]
> formula = `response` ~ `monitoredinfo` + `hs_emailinfo` + `uni_emailinfo` + `ll_fem` + `apt_equip` + `apt_size_h` + `apt_price_t`
> ssc = fixest::ssc(K.adj = TRUE, G.adj = TRUE)
> reg_vcov = "hetero"
> reg = feglm(fml = formula,data = dat,vcov = reg_vcov,family=binomial(link = "probit"),ssc = ssc)
...
1 (Intercept)     0.0966    0.231     0.419   0.675  -0.356      0.549 
2 monitoredinfo  -0.211     0.143    -1.47    0.140  -0.492      0.0695
3 hs_emailinfo   -0.0139    0.178    -0.0777  0.938  -0.364      0.336 
4 uni_emailinfo   0.0167    0.191     0.0874  0.930  -0.358      0.392 
5 ll_fem          0.0841    0.114     0.736   0.462  -0.140      0.308 
6 apt_equip      -0.181     0.159    -1.14    0.255  -0.491      0.130 
7 apt_size_h      0.560     0.407     1.37    0.169  -0.238      1.36  
8 apt_price_t     0.0483    0.0214    2.26    0.0241  0.00632    0.0902
> reg
GLM estimation, family = binomial(link = "probit"), Dep. Var.: response
Observations: 606
Standard-errors: Heteroskedasticity-robust 
               Estimate Std. Error   z value Pr(>|z|)    
(Intercept)    0.096631   0.230830  0.418626 0.675489    
monitoredinfo -0.211151   0.143199 -1.474526 0.140340    
hs_emailinfo  -0.013859   0.178472 -0.077654 0.938103    
uni_emailinfo  0.016723   0.191245  0.087443 0.930320    
ll_fem         0.084095   0.114330  0.735548 0.462006    
apt_equip     -0.180630   0.158607 -1.138856 0.254763    
apt_size_h     0.559700   0.407115  1.374795 0.169195    
apt_price_t    0.048258   0.021399  2.255139 0.024125 *  
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
Log-Likelihood: -328.5   Adj. Pseudo R2: 0.004102
           BIC:  708.2     Squared Cor.: 0.02213 


The R translation does not seem to drop the observations that Stata drops. Could the same behavior as in Stata be easily implemented in regtranslate?

I also added a file drop_perfect_predictor.R in regtranslate. But the functions are not yet implemented in our pipeline. Can you check the Stata docs / internet where Stata by default drops perfect predictors and a corresponding call in the R regression code generated by regtranslate. Add an argument in the regtranslate translation function whether that actually shall be done (by default TRUE if Stata does so to match Stata's behavior.)

Also discuss whether that all makes sense and explain your changes, beforee you write the code.
