...
The metaregBase & regtranslate framework can deal with abbreviations in variable names by matching them with columns in the data set. Can the stata2r package do the same? Are there also Stata commands outside regression varlist that can use abbreviations or glob expansions? If yes we should add the functionality. Basically all generated translated code calls a function with a data set. We could write code that does the variable expansion during runtime based on the data set. But that makes the generated code harder to read. Can you check whether that is neccessary? If yes propose conceptually, but in detail with concrete examples, how the generated code could look like, and how we would need to change the stata2r package. Ideally stata2r should not depend on metaregBase or regtranslate. In doubt the variable expansion functions from there should be duplicated / adapted. 
