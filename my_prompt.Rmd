...
I have an example where a variable in the regressen (here even the dependent variable) is of class Date. The pipeline currently throws an error when trying to run the R translation of the regression:

```
 library(fixest)
> rcmd = "feols"
> cc_cols = c("p_1_max", "size2", "sec_textiles", "sec_dyes", "wwg100", "coallignite", "reg_abd", "reg_sur", "inspections", "p_bin")
> cc_cols = intersect(cc_cols, colnames(dat))
> dat = dat[complete.cases(dat[, cc_cols, drop=FALSE]), ]
> formula = `inspections` ~ `p_1_max` + `size2` + `sec_textiles` + `sec_dyes` + `wwg100` + `coallignite` + `reg_abd` + `reg_sur` + `p_bin`
> ssc = fixest::ssc()
> reg_vcov = "hetero"
> reg = feols(fml = formula,data = dat,vcov = reg_vcov,ssc = ssc)
Error in feols(fml = formula, data = dat, vcov = reg_vcov, ssc = ssc) : 
  The left hand side must be a numeric matrix (or vector) with 1 column.
PROBLEM: it is not numeric (instead it is of type 'Date').
> class(dat$inspections)
[1] "Date"
```

Can we do the following. Check in regtranslate whether any of the regression variables is a Date / or Datetime that will be directly used as a number (not as fixed effects like if regression formula is like `i.datevar`). Only if yes let the translated R code call a function drf_datetime_regvar_to_num(dat, cols). If not we skip it.

Currently, I think we don't store in regvar and regxvar infos whether the variable class is Date or Datetime but store it as numeric when not used as a fixed effect. Would it be cleaner easier to store the info directly in regvar and regxvar? I don't know think and discuss.

Also directly using variables that are a Date or Datetime (POSIXct) seems a bit weird, because effect sizes are hard to interpret. If such a case is encountered, could you note it with a repbox_problem call? Just as message so that we are aware of that issue and maybe in a metastudy can check later whether that is indeed correct usage.

Please first discuss and explain. Then make a proposal of all required code changes.
